...
> 本文是 dsh-work 插件（host 工具 + 浏览器活动面板 + 对话流卡片）开发全过程的经验蒸馏。
> 覆盖 bundle 插件从骨架、host 面、client 面、构建安装到踩坑修复的完整流程，供 coding agent 直接照做。
> 参考实现：`dsh-work`（成品）、DSH 仓库 `packages/workflow/tool-workflow`（工具插件模板）、
...
npx -p @deepseek-ai/dsh dsh plugin --profile web add /absolute/path/to/dsh-work
