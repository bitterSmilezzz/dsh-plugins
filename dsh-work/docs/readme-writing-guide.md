...
> 提炼自 `dsh-work` 成品 README 的多轮迭代（功能/原理/UI/工具/安装/配置/使用/验证/限制全结构），并对照 DSH 仓库内包 README 的风格（`packages/preset`、`packages/bundle`、`packages/client/ui-workflow-run`：精炼、表格化）。

### 0. 语言与篇幅策略

- **独立插件项目**（面向安装用户，如 `dsh-work`）：中文为主，命令、工具名、标识符、字段名保留英文；解释性句子用中文。
