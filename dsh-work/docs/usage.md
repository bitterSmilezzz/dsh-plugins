...
本文档收纳 dsh-work 的详细使用内容：工作原理、Web UI 行为、工具一览、配置与已知限制。README 只保留简介与快速上手。

## 工作原理

`dsh-work` 复用 DSH 的能力接缝（capability seam），不依赖 workflow 引擎：
...
| Web server 路由注册 | 活动面板数据路由 `/plugins/dsh-work/state` + 鲸鱼图片静态服务（`webServer`/`httpServer` 双键兼容，见下） |
