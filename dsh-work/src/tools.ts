...
export function registerAgentTeamsTools(ctx: Context, config: ToolsConfig): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Add a durable member to the team. Member waits for messages and works tasks; provider/model only when the user explicitly requests a different route.',
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Create a task in the team. Dependencies must complete before the task is claimable; optional assignee still claims it first.',
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Claim a task for a member/yourself. Blocked until dependencies finish; captain may claim for assignee, member only assigned/unassigned.',
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Send a message to the captain or a teammate via durable mailbox; may live-deliver when online. No relay.',
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Team snapshot: members, tasks, status, outputs, and mailboxes (captain sees all; members own inbox).',
    parameters: {},
    output: {
