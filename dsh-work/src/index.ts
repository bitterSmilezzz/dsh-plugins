 ...
 * results.
 *
 * Installation (bundle): `dsh plugin --profile <name> add dsh-work`
...
0. Call agent_teams_activate once to enable the agent_teams_* tools; after that they are available for the rest of the session.
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Enable the AgentTeams multi-agent tools for this session. Call once before using agent_teams_create/add_member/remove_member/create_task/claim_task/update_task/send_message/status/delete.',
    parameters: {},
    output: {
      schema: { type: 'string' },
    ...
    async execute() {
      if (agentTeamsActivated) return 'AgentTeams tools already enabled.'
