 ...
 * polls the host `/plugins/dsh-work/state` route for
 ...
 * @module dsh-work/client/activity
 */
...
import { activityPanelExpandedForSession, dependencyFocusTaskId, relatedTaskIds, taskStages, } from "./activity-model.js";
...
const STATE_URL = '/plugins/dsh-work/state';
...
function TaskNode({ task, tasks, focused, dimmed, pinned, onPin, onHover, onFocusChange }) {
    ...
    return (_jsxs("button", { type: "button", className: css.taskNode, "data-task-id": task.id, "data-state": tone, "data-focused": focused, "data-dimmed": dimmed, "aria-pressed": pinned, title: `${task.id} · ${task.subject}（悬停高亮依赖链 · 点击固定）`, onClick: () => { onPin(task.id); }, onMouseEnter: () => { onHover(task.id); }, onMouseLeave: () => { onHover(null); }, onFocus: () => { onFocusChange(task.id); }, onBlur: () => { onFocusChange(null); }, children: [_jsxs("span", { className: css.taskNodeHead, children: [_jsx("span", { className: css.taskId, children: task.id }), _jsx("span", { className: css.taskBadge, "data-state": tone, children: taskStatusLabel(task.status) })] }), _jsx("span", { className: css.taskSubject, children: task.subject }), _jsxs("span", { className: css.taskRoute, children: [_jsx("span", { className: css.taskOwner, children: task.assignee || '待认领' }), task.dependencies.length === 0
