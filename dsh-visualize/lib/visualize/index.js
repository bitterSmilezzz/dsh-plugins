...
//#endregion
//#region src/tool.ts
const DESCRIPTION = "Render an interactive HTML card in the conversation. `create` (default): inline HTML fragment only, no doctype/html/head/body (the card supplies document, stylesheet, theme). `update`: `path` + one exact `old_str`/`new_str`. A workspace copy is saved. Load the `visualize` skill before first use.";
/**
	...
	return defineTool({
		...
		parameters: {
			action: {
				type: "string",
				enum: ["create", "update"],
				description: "`create` (default) renders a card from `fragment`. `update` patches the card at `path` via one exact `old_str`->`new_str`; use only for <20 lines, <5 places, <=4 per reply; re-create otherwise."
			},
			fragment: {
				type: "string",
				description: "create only, required: inline HTML body (markup+style+script, no skeleton)."
			},
			title: {
				type: "string",
				description: "Concise title. Default \"Visualization\" on create; required on update."
			},
			mode: {
				type: "string",
				enum: ["inline", "wide"],
				description: "Width: `inline` (default) or `wide` for side-by-side panels."
			},
			path: {
				type: "string",
				description: "update only, required: workspace path of the card to patch."
			},
			old_str: {
				type: "string",
				description: "update only, required: exact card text to replace; must appear exactly once (whitespace included)."
			},
			new_str: {
				type: "string",
				description: "update only, required: replacement text; empty deletes matched region."
			}
		},
		output: {
			schema: {
