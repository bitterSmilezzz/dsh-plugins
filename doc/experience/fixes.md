...
- **cordis `connection.rpc.handle` 的 owner 陷阱：第三方插件注册 RPC 通道会静默失败（2026-09-17，坑/契约）**：问题=dsh-notify 用官方 `connection.rpc.handle('/dsh-notify', handler)` 注册 host↔client 通道，返回无异常、`rpc.handle` 看似成功，但 route 完全不存在（请求落到 frontend-static fallback：POST 得 405、GET 得 404）。原因=**`rpc.handle` 内部用 `owner.webServer.register(...)` 挂 prefix route**，而 owner 取自 `get rpc() { const owner = this.ctx }`——service 的 `ctx` 经 cordis tracker（`tracker.property = 'ctx'`）在 traceable proxy 下被替换为**访问者 ctx**，该 ctx 的属性访问受 ReflectService 的 inject 检查约束；即使把 `webServer` 加进 `ctx.inject([...])` 仍抛 `Error: cannot get property "webServer" without inject`（实测：同一 ctx 上 `ctx.get('webServer')` 返回 present、探针 `webServer.register({kind:'exact',path:'/dsh-notify-probe'})` 立即生效 GET 200，唯独 `rpc.handle` 抛错——owner 解析路径与直接属性访问不同）。**解法**：改用官方明确支持的 `ctx.webServer.register({ kind: 'exact', path: '/api/<plugin>/<name>', handler })`（exact route 可覆盖 `/api` prefix 共享通道，asr-voice 的 `/api/asr-voice/*` 即此模式），信任围栏**不要自建**——直接复用 `connection.requestRejection(req)`（Host/Origin 检查 + 浏览器认证，与 `/api` 共享通道同一套判定，返回 401/403/undefined），注册随子 fiber 回收（`ctx.effect`）。**坑**：① 官方**没有任何生产插件用 `rpc.handle`**（只有测试用），第三方插件照搬会踩这个坑；② 排查手法值得复用：`ctx.inject([...], cb)` 内加 `console.log` 到 stdout（DSH 前台运行时就是终端输出）+ 探针 route 对照（`webServer.register` 通不通）+ 无 cookie curl 对照（`/api/*` 一律先过围栏 401，用带 cookie 的页面 fetch 才能区分"route 存在"与"prefix 通道拦截"）；③ `frontend-static` fallback 的 405（非 GET/HEAD 无匹配 named route）是"路由不存在"的可靠判据，比 404 更能区分（GET 404 可能只是静态文件缺失）。**验证**=带 cookie 端到端：`POST {visible:true}`→204、`POST {visible:'yes'}`→400、`GET`→405、无 cookie→401；页面 `performance.getEntriesByType('resource')` 见 client 自动上报 `/api/dsh-notify/presence`（fetch，页面加载后 169ms）。可复现?是（旧写法 route 必不存在；新写法三档状态码齐现）。
- **dsh-model-selector 0.2.0 多视角优化轮：坐标系混用 / 未取整值无归一化出口 / Unicode 折叠改长度 / rank 0 冒充「最强档」（2026-09-17，修复/审查/无障碍）**：问题=用户要求「多方面、多视角的深入、完整优化」。三个并行只读视角（React 正确性 / 无障碍 a11y / 测试与发布链路）得 ~30 项可定位问题，修 25+ 项、测试 75→111。按根因分六类：① **坐标系混用**：菜单水平钳位函数 `dmsMenuLeft` 的入参/返回值都是**视口坐标**，却被直接写进 `.dms-root`（`position: relative`）内 `absolute` 元素的 `left` —— 参考系相差 root 左缘。窄窗口下菜单整体右移 `rootRect.left`；页面横向溢出（`rect.right > innerWidth`，正是该钳位要处理的场景）时算出 `innerWidth - margin - menuWidth` 再叠 rootLeft，**整幅画到屏外**。修=调用点减 `rootLeft`，并把坐标系写进函数注释。② **刻意不取整的中间值缺归一化出口**：`dmsPointerRaw` 返回分数（拖动中途可落两档之间），而 `commit()` 的「同档早退」跳过 preview 归一化 → 旋钮/进度条/画布停在两档之间，读数却已显示整档；同步 effect 因依赖未变不会纠正（**只有指针路径有**，键盘路径传整数 index）。修=早退分支仍 `setPreview(index)`。③ **大小写折叠会改变码位长度**：`name.toLowerCase().indexOf(q)` 的下标拿去切原串。`'İ'.toLowerCase()` 是 `'i̇'`（i + U+0307 **两个码位**）→ `İstanbul` 用 ASCII 小写名 `istanbul` **根本搜不到**（漏检），长度变化时下标还会错位。修=`dmsFold` = NFKD 分解 + 去组合标记 + toLowerCase 做匹配；高亮**只在折叠后长度不变时**给区间（宁可不高亮，也不能画到别的字符上）。④ **rank 0 冒充「最强档」**：`maxEffortOf` 对并列 rank 0 取首个，而适配器自造档位（`turbo`）rank 也是 0 → 被当最强档提交，可能被宿主拒（用户看到「切换失败」）。修=`dmsEffortRank` 区分「已知 rank 0（off）」与「未知」，未知 id 不自动提交（`dmsChoosePlan`）。⑤ **a11y 四连**：选中行 `:focus-visible` 与选中态同 token → roving 默认落点恰是选中行 → **键盘焦点完全不可见**（WCAG 2.4.7）；组头恒 `tabIndex=-1` 而「全部折叠」时 roving 默认落点正是组头 → **Tab 跳过整个列表**（活动行键落空）；搜索框缺 combobox 语义（结果容器搜索态应 listbox/option）；live region 插入式渲染（首次播报不可靠）、`aria-busy` 挂在外层 `.dms-menu` 会罩住 live region 压制播报、空态时 `aria-controls` 悬空。⑥ **WebKit 的 IME 确认键**：`compositionend` 后补发的 Enter 带 `isComposing === false`（WebKit bug 165004），只有 `keyCode === 229` 能识别 → 中文候选上屏直接选中首个命中并切模型。同类还有：RPC 在途时用户已关菜单/移走焦点，成功回调仍 `close(true)` → **抢焦点 + 清掉新输入的搜索词**（改为只在菜单仍打开时关，open 经 ref 读最新）；busy 期 `onBlur` 豁免 → 留下「焦点已在菜单外、Escape 也关不掉」的悬挂菜单（keydown 不再冒泡到 root）；点击菜单内非可聚焦区域（内边距/标签/分组间隙）浏览器把焦点收回 body → 被当成「离开菜单」误关（改为 relatedTarget 为 null 时只在 `document.hasFocus() === false` 才关，并给 root 加 `tabIndex=-1` 承接焦点）。**坑**=①「刻意不取整」的返回值必须在**每条**出口归一化，早退路径最容易漏；②Unicode 大小写折叠不是等长变换，任何「折叠串下标切原串」的写法都要先验证长度；③判据用字面量（`max === 'off'`）而不是 rank 时，非规范输入会走错分支；④`define`/`intro` 注入的构建标记在 rolldown 下会被 tree-shake（见 governance 同日条）。**验证**=双 program typecheck 绿；`node --test` 75→111 全绿；**真机 web profile**（3080，插件以 symlink 装进 profile，bundle 即本仓库 `lib/client.js`）实测：combobox 角色生效、搜索 `mimo` 命中 `MiMo-V2.5`（折叠匹配 + 高亮分段）、Escape 分层清词不关菜单且焦点回搜索框、菜单内点击不关菜单（焦点落 root）、组头承接 `tabIndex=0`、焦点内描边 `inset 0 0 0 2px`、Escape 关闭还焦 trigger、菜单完整在视口内且向上弹。**未真机验证**：水平钳位（DSH 在 320px 视口不渲染模型 seat，且 CDP 视口模拟会让页面 React 根卸载）、IME 守卫（需 Safari + 中文输入法）。可复现?是（每项都有对应单测或真机判据）。

- **dsh-asr-voice 0.3.3 多视角优化轮：原生 API「自己结束」路径 + 信任围栏端口 + 三份副本漂移（2026-09-17，修复/安全/审查）**：问题=用户要求「多方面、多视角的深入、完整优化」。按四视角（client 正确性 / host 安全 / UI-a11y / 架构-发布契约）并行只读审查得 ~35 项真实问题，分四组实施后共修 30+ 项、测试 248→318。三个根因类型：① **「原生 API 自己结束/自己失败」的路径最容易被漏**——(a) Web Speech 的 `continuous` 会话会在用户静音数秒后**自行触发 `onend`**，整段录音链路把它当「用户按了停止」直接 `settle()`：识别器已死、用户继续说一个字都进不来，UI 却仍显示「录音中」（呼吸环还在动，只有频谱不动）→ 后半段静默丢失；(b) AudioWorklet 的 `addModule` 每次会话都用**新 blob URL** 注册**同名** processor，而规范规定「每个 AudioContext 只有一个 worklet global scope，`registerProcessor` 对已存在的名字抛 `NotSupportedError`」→ 同一页面**第二次**实时对话必挂，且错误文案指向「浏览器不支持」（完全反向）；(c) 按住说话的 `keyup` 无脑 `toggle()`，而录音可能已自行结束（静音自动停止 / `no-speech` / 到达时长上限）→ 落在 transcribing 会 `cancel()` 掉在途转写（刚口述的文本直接消失），落在 idle 会重新开录（键已松开，麦克风录到上限）；(d) 同类的还有 `recognition.start()` 抛错被 `catch {}` 吞掉（不会再有 `onend`，链路变聋而 UI 仍在「聆听中」）。解法=(a) 整段引擎补与实时引擎同款的**冷却重启**（`RESTART_DELAY_MS=120`、连败 3 次判死并报 `network` 交给 auto 降级）、`no-speech` 按「已说过话就续听」区分；(b) worklet 模块 URL 提到**模块级只创建一次**（`addModule` 对同一 URL 幂等；**不可 revoke**，否则下次 addModule 直接失败）；(c) 松键改为状态感知的 `releaseHold()`（只收尾 recording 这一种状态），并补 `blur`/`visibilitychange` 复位；(d) `start()` 抛错走冷却重试 + 连败判死。② **安全围栏的「同源」必须含端口**——`isTrusted` 只比主机名（`originName === hostName`），于是 `http://localhost:5173` 上的任意本机页面（dev server / 预览服务 / 本机任何 web 应用）POST 到 `http://localhost:3080/api/asr-voice/*` 被判为可信——`Sec-Fetch-Site` 此时是 `same-site` 拦不住，可借宿主代理花用户的 API key；测试表里那条 `{ host: '127.0.0.1:3080', origin: 'http://127.0.0.1:9', want: true }` 正是漏洞本身被固化成「预期行为」。修法=比较 **scheme+host+port**（默认端口两侧归一化），并同步三仓共用的围栏表（另两仓需自行同步）。③ **同一规则的副本会漂移，且漂移是「正在发生」而非理论风险**——(a) 设置默认值三份（host schema / client `DEFAULTS` / README 表），README 已漂移 5 处（「与麦克风并列第二个按钮」在 0.3.0 已合并成单按钮长按、`realtime.hotkey` 默认值写空、`tts` 缺 `cloud` 档、`provider`/`ttsVoice` 两行整缺、「真云端 provider 仍是后续阶段」而它早已实现）；(b) 静音阈值两份（`pcm.ts` 的 `SILENCE_PEAK_FLOOR` vs cloud 引擎硬编码 `0.005`，注释还写着「同一判据」）；(c) **密钥引用派生两条路径**（读取用 `providerView(row,'provider')` 兜底 id、迁移用原始行）→ 行缺 `id` 时迁移写进 `ASR_VOICE_CUSTOM_API_KEY`、读取查 `ASR_VOICE_PROVIDER_API_KEY`，明文抹掉后**密钥彻底不可达**。解法=统一派生入口 + 加**机器守卫**（schema↔DEFAULTS 递归比对测试、产物 external 白名单扫描、README 表 vs schema 机器核对），别指望「注释里写了单一真相」就能防漂移。可复现?是（围栏：本机 5173 页面 POST 3080 应 403；worklet：同页开两次 `engine=segmented` 对话，第二次必失败→修复后正常；录音 onend：默认引擎口述半句后停顿 8 秒再继续说，修复前后半段丢失；迁移：`{id:'',preset:'custom',name:''}` 行存 key 后迁移，旧版会写进 `ASR_VOICE_CUSTOM_API_KEY`）。
- **dsh-notify 0.2.0 多视角优化轮：删防重叠探测 + Linux 支持 + 文案本地化 + 会话标题 + 聚焦抑制 + 卡片双契约（2026-09-17，修复/优化）**：问题=用户要求"多方面、多视角的深入、完整优化"。调研后确认 8 项真实缺陷并全部修复：① **防重叠探测（overlap/probeServices）是零命中死代码**——实测官方 251 个包 grep 无任何 `notification` service，探测的 6 个名字纯属猜测；更糟的是**误命中即静默停止通知**（只弹一次提示），通知类插件最糟的失败模式 → 整机制删除（含两个配置项与设置行；schemastery 保留未知字段，老配置残留无害）；② **CHANGELOG 对 overlap 的描述与实现完全错位**（0.1.10 写"检测到屏幕被其他应用独占（如全屏演示、投屏）时自动暂停"，实现是探测 cordis service 名）；③ **设置卡片用了不存在的 token** `--dsw-alias-state-danger-fill`（官方只有 `state-error-*` / `label-error`）→ 恒走硬编码 fallback 色；④ **卡片视觉与官方 PluginCard 有偏差**（1px border-l2/radius 12 vs 官方 0.5px border-l4/radius 16/无打开态/无 focus-visible）→ 全参数对齐 + chevron 改官方 `IconChevronDownOutline14`；⑤ **locale 文案 masterDesc 与实现不符**（"macOS 需已装 terminal-notifier"，实际 osascript 为主、terminal-notifier 只影响可点击跳转）；⑥ **README 完全没写 overlap/probeServices**（功能列表缺项）；⑦ **locales.ts 注释声称 en 字典 checked complete 但无任何测试**；⑧ **`package.json` 的 `./client` types 指向从未生成的 `lib/types/client/`**（tsconfig.client.json 是 noEmit，tsdown dts:false）→ build 补 `tsc -p tsconfig.client.json`（emitDeclarationOnly）产出。**新增能力**：Linux（notify-send，PATH 探测 + `--` 终止符防 GOption 误解析）、通知文案本地化（host 读官方 locale 偏好，30s TTL 缓存——`settings.describe()` 会为每个 namespace 克隆 base/user，别在热路径裸调）、正文优先会话标题（官方 `sessionTitle.get(session)`，findLast 折叠 O(n) 微秒级）、聚焦抑制分级（client 经官方 Connection RPC 上报可见性；**只抑制轮次/会话完成，审批与错误始终送达**——审批是阻塞性的且官方审批 UI 只在对应会话内出现，用户在看别的会话时看不到）。**坑**：① 聚焦抑制必须设 TTL 兜底（client 30s 续期 / host 75s 保鲜期），失败方向选"宁可多通知，不可静默失效"；② `findOnPath(name, pathEnv = process.env.PATH)` 显式传 `undefined` 会触发 JS 默认参数回落 process.env.PATH——测"PATH 缺失"只能用 `''`；③ CJS 内建（node:child_process/node:fs）的 ESM 具名导入对被测模块是**加载时快照**，测试里运行时替换 `fs.writeFileSync` 属性不生效（须在 import 被测模块前就位）。**验证**=双 tsc 零错 + build 成功 + 测试 58→92 全绿（新增文案/聚焦/Linux/Windows/配置一致性；8 个变异全部被捕获）+ 伞校验 21/22（唯一 WARN：未声明商城分发意图）+ 浏览器实测新 bundle 加载。可复现?是（每项缺陷都有对应测试或 grep 判据）。
- **「DSH 要不要升级」的判据链 + 官方原语不可等价替代的三个反例（2026-09-16，核查/兼容性）**：问题=用户要求「更新 DSH 后做插件兼容性优化、更新与发布」。原因/做法=①**先证明「无需更新」**再谈优化：`git ls-remote origin refs/heads/master` 做真实网络复核（＝本地 HEAD `0d1f50007f`，远端只有 master 一个 head），最新 tag `dsh-v0.1.6-alpha.1`，npm CLI 与抽查 4 个官方包的 `dist-tags.alpha` 同为 0.1.6-alpha.1，再叠加**两条时间线**——构建产物 `apps/cli/lib/bin.js` mtime（09-15 19:53）晚于 HEAD 提交时间（09-15 11:16）、运行中的 `dsh web` 进程启动时间（09-16 00:31）晚于构建 → 判定「已是最新，无可更新」。只看版本号或只看本地 `origin/master` 引用都会误判。②**发布态复核可离线强验证**：`npm pack <pkg>@<ver>` 解包后与 `git archive <tag>` 逐文件 sha256 比对（18/31/58 个文件全等、`tarball_only=0`）；「只在 tag 里」的文件属正常（`files` 白名单）。③**官方原语不是自绘控件的通用替代品，三个反例**：(a) `useAnchoredPosition` 的 `place()` 恒 `left = rect.left`（仅越界钳位）+ `side` 由调用方写死 + 返回 `fixed` 坐标，而 dsh-model-selector 的菜单默认**右缘对齐 seat**（`styles.ts` 的 `.dms-menu{right:0}`，仅放不下才 `{left, right:'auto'}`）、需按 trigger 上下空间自动选边、且是 `.dms-root` 内 `absolute` 锚定 → 换成官方 hook 是**行为回归**而非重构；(b) 官方 `Menu` 把条目固定渲染为 `<button role="menuitem">`，**装不下** effort 滑杆与搜索框；(c) 官方 `Switch` 可且应当替换自绘开关（notify「音效」行本轮对齐，0.1.14）。结论=**不硬改，把判据写成代码注释 + 用测试钉住契约**（model-selector 加座位契约测试，0.1.20，`lib/client.js` 逐字节未变＝无运行时变更）。④**另一类重叠要按契约先问用户**：官方 `ui-model-selection` 自带 `conversation.input.model` 座位与完整 `ModelSelect`（未传 priority＝默认 0），插件靠 `priority:-1`（single 槽按 priority 升序取最低者渲染）遮蔽它——**座位层真重叠**，自 2026-08-31 长期存在、属有意设计；按伞仓库契约先通知用户后才动，本轮先只做「记录 + 钉子」并通知用户；用户决策=**保留遮蔽设计**（single 槽只能有一个渲染者，官方组件没有 effort 分组/搜索、也没有可叠加增强的扩展点，改成复用官方组件＝丢掉插件价值），并把这条隐式依赖升级为**双侧钉子**：插件侧 `test/plugin-contract.test.mjs` 钉 `priority:-1`，**依赖侧** `test/official-seat-shadow.test.mjs` 钉「官方产物仍不声明 `priority`」（官方当前对该座位 0 处 `priority`；负控已验：给产物注入 `priority: -1` 后正则命中）。官方一改（声明优先级或 single→chain）测试即红，而不是等用户发现模型选择器变回了原生 UI；同时把「提交 DSH-Store 时该遮蔽需要解释（走 slot shadowing 语义，未禁用/替换官方 entry）」写进插件 README「已知风险」。可复现?是（ls-remote／npm pack 比对／读官方 hook 与 Menu 源码／对官方产物跑那条守卫测试即可逐条复验）。
- **pnpm peer 覆盖陷阱复发：旧解法在 pnpm 11 上失效，须临时挪开 pnpm-workspace.yaml 才能修复（2026-09-14，修复/依赖）**：问题=dsh-computer-use 跑 vitest 时 7 个测试文件加载失败（0 test）——`Cannot find package '@deepseek-ai/dsh-scope' imported from …/dsh-tools/lib/index.js`（另有 dsh-storage）。**识别信号：统计显示 `7 files failed` 但只有 `3 tests failed`**，文件数远多于用例数＝有文件根本没加载起来，只看用例数会把「半个套件没跑」误判成小问题。原因=仓库根 `pnpm-workspace.yaml` 的 `autoInstallPeers: false`（发布契约，`tests/package-layout.spec.ts` 断言精确三行，不可改）；**只要跑一次裸 `pnpm install`，pnpm 就按它「纠正」lockfile**——头部改 `false`、`dsh-tools` 重解析到不带 peer 的变体（`c84620f8…`）、顶层软链指到该空变体，二级 peer `dsh-scope`/`dsh-storage` 从依赖图消失。computer-use 以 `link:` 挂进 web profile，坏 node_modules 会**顺真实路径传染 `dsh web`**，不只是测试问题。解法=①**旧解法已失效**：`pnpm install --config.auto-install-peers=true` 与 env 形式 `npm_config_auto_install_peers=true pnpm install` 在 **pnpm 11.20.0 上均无效**——报 `Already up to date` 却照旧把 lockfile 写成坏形态（该仓库 `packageManager` 钉 pnpm@11.20.0，另三仓库跑 pnpm v12.4.1，CLI/env 覆盖在那三个上有效；**不要假定同一解法跨 pnpm 大版本通用**）；②有效做法是**临时挪开 workspace 文件**让 `false` 无处生效：`mv pnpm-workspace.yaml /tmp/moved && rm -rf node_modules && pnpm install && mv /tmp/moved pnpm-workspace.yaml`，随后 `shasum -a 256` 复核 workspace 文件与挪开前一致；③判据：`readlink node_modules/@deepseek-ai/dsh-tools` 应指向带 peer 的变体（好=`…_7f9e3027…`／坏=`…_c84620f8…`），且 `git diff --quiet pnpm-lock.yaml`（修好后 lockfile 与 HEAD 逐字节一致）。**坑**=①**误判警告**：`dsh-scope`/`dsh-storage` 在仓库**顶层解析不到是正常的**（二级 peer 只存在于各依赖方的 `.pnpm` 变体目录里），用根目录 `import('@deepseek-ai/dsh-scope')` 判断修复与否必然假阴性；②修复前后对比：文件 7 failed → 1 failed/18 passed，用例 3 failed/98 passed/116 → 2 failed/**168 passed**/185——**用例总数 116→185 才是「加载失败的文件回来了」的直接证据**。可复现?是（任何带 `pnpm-workspace.yaml` + `autoInstallPeers: false` 的仓库跑一次裸 `pnpm install` 即复现）。
- **dsh-asr-voice LLM 优化空返回：opencode-go 网关要求 x-opencode-session，插件直呼 ctx.llm.stream 不带 sessionId 被 400 拒（2026-09-10，修复/LLM 路由）**：问题=用户语音「LLM 重写」优化报「优化失败: LLM optimize failed: model returned no text」，默认目标模型（modlens-opencode-go/deepseek-v4-flash）下必现。原因=llm-pi-ai 适配器**只在 `GenerateOptions.sessionId` 存在时**才把会话头（provider 配置的 `sessionHeader: x-opencode-session`）注入请求（llm-pi-ai adapter.ts 的 requestHeaders 分支）；聊天回路由 agent-loop 打真实 DSH SessionId，而插件直呼 `ctx.llm.stream` 没带 → opencode-go Console Go 网关 400 `MissingSessionID`（响应体明说 missing x-opencode-session），finish kind=error 但失败细节被插件吞成笼统「model returned no text」。解法=①optimize.ts 每次调用附一个随机 sessionId（实测网关接受任意值、按需自建会话；deepseek-official / xiaomi 等无 sessionHeader 的 provider 忽略该字段，无副作用——`GenerateOptions.sessionId` 是 dsh-brand 名义类型且 dsh-llm 未导出具名类型，插件非 dsh-brand 直接依赖，只能在调用处 `as unknown as Parameters<typeof ctx.llm.stream>[0]` 断言）；②顺带把 finish 的 error/aborted `failure.message` 透出到错误文案（240 字符截断），同类网关错误不再隐身。验证=live host 对照实验（动态插件注册 /api/asr-probe 直呼 ctx.llm.stream 抓 chunk 级证据）：无 sessionId → error finish 400 MissingSessionID；带任意新 id → 全文 + finish stop；deepseek-official / xiaomi 加 sessionId 对照全部正常出文（dimagent-oauth 的 402 是额度问题与 sessionId 无关）；修复后 210/210 测试 + 双半区 build 过，commit 3d08f9e（asr-voice，host 半区改动需重启 dsh web 生效）。可复现?是（不带 sessionId 直呼 llm.stream 打 opencode-go 系 provider 必现 400 MissingSessionID，带 sessionId 即正常）。

- **dsh-model-selector 第三轮键盘/无障碍收尾：中断子代理复活自提交 + 恢复任务必须逐项核对原清单（2026-09-06，修复/键盘无障碍）**：问题=修复子代理执行第三轮 8 项清单时中断，主会话接手时工作区有未提交改动；逐项核对发现 item5（reduced-motion 补 `.dms-effort-slider` filter 180ms 与 `.dms-effort-canvas` filter 140ms 两条 transition:none）实际没做，且随后**该子代理自行恢复并已提交**（3c2a7a2，含 1/2/3/4/6/7 项 + roving 9 测试，73/73），我的 item5 改动悬在已提交 HEAD 之上。解法=①从 OpenViking 会话归档 `sessions/*/history/*/messages.jsonl` 找回**完整任务清单**逐项核对——profile.md 记忆被截断（只有第 1 项），会话原文才是真相；②补 item5 两行（styles.ts:568 reduce 块加两个选择器）；③对未 push 的 HEAD 用 `git commit --amend` 合入补漏项并改回规格提交信息，保住「一个 commit」契约；④全量重验 73/73 + 双半区 tsc + 完整 build（host tsc + tsdown + client d.ts）。**坑**=①**恢复中断任务先 `git log`/`git reflog` 确认状态再动手**：工作区「未提交」可能瞬间变「已提交」（子代理复活续跑），按旧快照 diff 会白做；②清单完成度不能凭 diff 印象判断——reduced-motion 这类 2 行小项最容易漏，逐项 grep 现码核对（当时 styles.d.ts 的 reduce 块逐字对照旧块即现形）；③`plugin-contract.test.mjs` 的 d.ts 新鲜度测试会因 src 新于 lib 变红，改 src 后必须全量 build 再跑测试，build 与测试顺序不能反。可复现?是（清单逐项 grep 现码即可复验漏项）。
- **dsh-notify 二轮审查四项低优先项：deep-link TDZ 防御 / summary 截断声明收紧 / sessions 注入解耦 / 手写类型换官方 merge（2026-09-06，修复/审查轮）**：问题=一轮修复后的遗留低优先项，按「做掉或记录放弃理由」处理。解法=①**deep-link.ts TDZ（做掉）**：上轮「timer 回调时 unsub 早已初始化」的复核结论漏了 **subscribe 同步抛错**路径——抛错后 unsub 停在未初始化、timer 已注册，15s 后回调访问即 TDZ ReferenceError 且 effect 无清理函数（timer 泄漏）；改 `let unsub` 先声明后赋值 + timer 回调判 `unsub === undefined` 则跳过（不清理 URL，刷新可重试）+ subscribe 包 try/catch（立即清 timer 返回空清理，不抛给 fiber）。②**summaryOf 组合字符截断声明过强（方案 b，放弃改实现）**：`Array.from` 按码点截断会劈开基字符+变音符（á=a+U+0301）与 ZWJ emoji 序列——弃 Intl.Segmenter：需浏览器回落路径且行为变更只修展示性瑕疵，收益不成比例；改 docstring「≤max 字符/输出永远是完整字符」→「≤max 码点，边界可能劈开组合字符，仅影响尾部展示」，另把 notify-policy.test.mjs 里过度声明的测试名/注释改准确 + 新增奇数边界行为钉（max=4 → `a\u0301a…`）。③**client inject 硬依赖 sessions（做掉，改可选）**：sessions 仅 deep-link 打开会话用，硬注入让整个 client 半区（设置卡/音效/词条）等会话控制器就绪——从 inject 移除，deep-link 改 `ctx.get('sessions')`（缺失防御与「不清理 URL」语义原有，行为不变）。④**index.ts 手写 NotifyHostContext（做掉，改官方 merge）**：`import type {} from '@deepseek-ai/dsh-settings'` 拉官方 SettingsProvider merge，register 返回 `SettingsScope<T>`、get() 直接给 schema 推断类型——删 6 行手写类型 + `as unknown as {...}` 强转 + 7 处 `?? 默认值`（schema 全字段 default 恒完整形态），apply 签名收 `Context`，改动 <20 行。**坑**=①**源码 `window.setTimeout` 与裸 `clearTimeout` 混用**（浏览器同一对象）：node 测试里裸 clearTimeout 是独立全局，假 window 只替 setTimeout 会漏记 clearTimeout 调用（断言莫名红）——必须连 `globalThis.clearTimeout` 一起替换并在 finally restore；②TDZ 测试是行为钉：subscribe 抛错 mock 下旧实现必崩（timer 回调 ReferenceError）、新实现绿；③tsdown 产物字符串用双引号（`ctx.get("sessions")`），grep 核对 bundle 用单引号 pattern 会假阴性。验证=40/40（35 旧 + 5 新：deep-link 4 例 effect 流程——subscribe 抛错/超时 unsub/快照即开/服务缺失 + notify-policy 奇数边界 1 例）+ 双 tsc 零错 + tsc host+tsdown build 成功；commit 0d38225（未 push、未改版本号、未动 README）。可复现?是（subscribe 抛错 mock 即触发旧实现 TDZ 崩）。
- **三插件并行审查修复轮（2026-09-06，修复/审查轮）：asr-voice 9 项（3 高 6 中）+ model-selector 7 项（5 中 2 防御）**：问题=用户「看看几个插件做优化」，并行三 subagent 审查 + 三 subagent 实施（review→fix 分离，审查只读、实施按清单落地）。**asr-voice（1589bf4）**：①**RealtimeHost 无 fiber 级清理（高）**——卸载/热重载时活动会话泄漏：SSE 心跳 setInterval/idle timer/provider WS 全脱离 fiber，浏览器麦克风最多再跑 maxSessionMs=10 分钟才自愈；加 `dispose()`（遍历 sessions 逐个 closeSession，幂等），index.ts effect disposer 在注销路由之外调用；②**SSE PENDING_CAP=64 溢出 shift 丢最旧 final（高）**——违反「final 必达」契约；改溢出先 pop 队尾 partial，仅全队都是 final 才丢最旧；③**dashscope WS CONNECTING 态静默丢帧（高）**——会话开头 100-500ms 语音帧丢失首句缺话头；加 32 条有界缓冲（满丢最旧），onopen 先 sendSessionUpdate 再按序冲刷，fail() 清缓冲；④上行泵队列无 cap（镜像 host 64）；⑤play() rejection 未捕获（catch 放 finally 前）；⑥recorder abort() 后 stop() promise 永不 settle（挂起 stopPromise 强制 resolve('')，resolve 空结果不触发正常完成路径）；⑦registerTtsRoute ctx:unknown 改 Context（删 as never）；⑧LIST_MODELS_TIMEOUT_MS 常量导出复用；⑨announce() 循环内 try/catch 隔离。**model-selector（b9ce4ff）**：①外部点击关闭绕过 close()（搜索词/notice 残留）——dismiss 回调包 close() 清态；②choose() select 无超时护栏（挂起永久锁死交互）——仿 effort commit 12s settled 定时 Promise + toast；③commit 12s 定时器卸载后存活——timer 进 ref + cleanup clearTimeout + mounted 守卫；④迟到成功采纳判据（committed===previous）分不清「无新提交」与「新提交失败」——引入单调递增提交 epoch 双条件判定；⑤hits 在 layout effect 依赖里每次击键做布局读——hits 非空提前 return。**坑**=①**「契约 vs 注释」分开审查**：notify 注释声称 notifyProbe service 暴露给 client，实际无 provide（文档虚构）——「实现或删注释」选删注释（注册 service 需跨 fiber 基建，避免过度工程），注释改准确即可；②**WIP 先落 commit 再优化**：三个插件工作区都有用户未提交的大改（model-selector 滑杆重构/notify 防重叠探测/asr-voice 依赖对齐），先各自 commit（用户工作保底）再在其上实施，避免优化 diff 与用户工作混在一起无法回滚；③**审查 subagent 与实施 subagent 分离**：审查只读产出「文件:行 + 建议改法」清单，实施按清单落地并自验（test+双 tsc+build），父代理只做交叉验证与 push——大插件（asr-voice 9k 行）全文精读不进主上下文；④测试先绿后改：asr-voice 193、model-selector 59、notify 31 基线全部先确认再动手。验证=asr-voice 196/196（+3：dispose/SSE 溢出保序/CONNECTING 缓冲）、model-selector 61/61（+2：epoch 误采纳防御）、notify 35/35（+4：macOS 兜底链）；三仓双 tsc 零错 + build 成功 + 伞校验 20/20；各 1 commit 已 push。可复现?是（三 commit：1589bf4/b9ce4ff/2e6b0db；溢出测试在旧实现必挂）。
- **dsh-notify 审查轮五项修复：macOS 兜底链缺口/探测文档虚构/动态化/scope 清理/音效死代码（2026-09-06，修复/审查轮）**：问题=评审确认 5 项（1 高 4 中）。解法=①**macOS 兜底链缺口（高）**：terminal-notifier 存在但 exit 非 0（macOS 26 NSUserNotification 点击 API 失效）时不触发 osascript 兜底、通知静默丢失——新增 `registerNotifierFallback`（防双发标志位：error 事件与 exit 非 0 合计只兜底一次；exit code 0=成功发送不兜底，信号杀死 code null 视为未发送），Silicon/Intel 两个 spawn 薄封装改走该注册；②**notifyProbe 文档虚构**：index.ts 注释声称暴露只读 service 给 client 卡片，实际无 provide、卡片无探测 UI——选 b 改准确（探测状态仅供 host 自动暂停判定 + 翻转边界发系统提示，不注册 service），system-notify.ts 同段注释一并改；③**probeServices 仅 apply 时快照**：并入 host NotifyConfig + notifyConfig() 每次读取（参考既有每次读取模式），applySystemNotify 删独立参数改 `configOf().probeServices`，改 settings 即时生效，首次探测包 try/catch；④**client/config.ts disposer 不彻底**：disposer 除 unsub 外重置模块级 notifyScope（identity 校验防误清新 fiber 绑定，卸载后 setConfig 退化只更新本地快照）+ scope.set 同步抛错 try/catch 兜底广播 config-error；⑤**sound.ts 死代码**：grep 证实 playSound 唯一调用是设置卡试听按钮（approval/question/turn pattern 与 300ms 节流表无事件触发路径）——删 pattern 集/节流表，playSound 去 kind 参数，保留 sessionDone 三连音为试听音。测试=新增 test/system-notify-fallback.test.mjs 4 例（exit 非 0→osascript 兜底+exit/error 防双发、error→兜底+随后 exit 不双发、exit 0 不兜底、无 openUrl 直走 osascript）。**坑**=①**Node 26 已移除 node:test 的 mock.module、mock.method 无法重定义内建模块命名空间**（Cannot redefine property）——但 CJS 内建（node:child_process/node:fs）的 ESM 具名导入是**活绑定**，`createRequire` 改写 CJS exports 属性即可注入替身（实测生效），比 loader 钩子轻；②替身 child 必须补 `unref()` 面，否则 spawn 封装同步 catch 当兜底通道触发、测试全部误判「双发」；③mock 内建必须与被测模块 import 同进程——新建独立测试文件（node --test 每文件独立进程），不能塞进既有 system-notify.test.mjs。验证=35/35（31 旧+4 新）+ 双 tsc 零错 + tsc host+tsdown build 成功；commit 2e6b0db（dsh-notify，未 push、未改版本号）。可复现?是（测试注册假 child 触发 exit/error 即可复验防双发）。
- **dsh-asr-voice 高度精简：9610→8916 行（-7.2%），行为零变化（2026-09-03，重构/精简）**：要求=不牺牲性能/不减功能/纯精简。解法按杠杆排序：①JSDoc 多行块折叠单行（-250，≤3 行块合成单行，脚本跳过模板串内部与 JSDoc 内反引号干扰——跟踪模板串状态时 **JSDoc 块内不更新**）；②死代码 `timeline`/`ticker`/`TimelineHandle`（animate.ts，无调用方无测试，-78）；③重复逻辑收编 6 组（-76）：normalizeKey 三合一（hotkey.ts 导出共用，注意 settings-card 的修饰键过滤语义移入 keyCombo 前置）、postHostApi（optimize/transcribe 共用 POST 骨架：外部信号桥接+超时+{ok,text,reason} 解析+AbortError→timeout 转换）、guardRoute（http.ts 统一 403/405 守卫收编 6 文件 10 处样板）、quantileOf（rms-floor 两份插入排序+分位）、pickActiveProvider（config 草稿/快照 active 供应商）、closeSegment（fake provider 段闭合）；④LocaleKey 189 键每行一个→每行 6 个（200→35 行，锚点注释保留）；⑤CSS ≤4 属性规则单行化 35 条（660→535，样式等价）。**刻意不动**：~320 条「为什么」级行注释（代际守卫/幽灵录音/Safari 坑）、settings schema 字段重复（声明与文档不分离）、speech-out 两 sink 外壳（差异面大于共享面，提取更绕）。验证=typecheck+build+test 177/177 全绿、jscpd 1.23%→0.69%、bundle 体积持平（246.6KB）、clone tag 发布态 validate 20/20 + imports 0 缺失、Release 已发。坑=①jscpd 首跑即知全仓克隆分布（13 处 105 行），比人眼扫准；②行注释清理收益不成比例（抽样 54 条全是竞态/时序解释），温和模式直接跳过；③脚本批量改联合类型要锚定注释分组语义，两轮修正后才语法正确。可复现?是（jscpd + node --test 即可复验）。
- **dsh-asr-voice 五轮收口：transcribe 干净、其余面结构级扫描合格，优化系列判定「找不出项」（2026-09-02，审计/收口）**：问题=继续优化。解法=transcribe.ts（host 转写代理）全文复审——超时/信任围栏/key 三级解析（settings 遗留→credentials→env）/诊断留存上限（100 个、时间戳名排序删最旧）/双通道错误形状兼容全部在位，零改动；config.ts/voice-button.tsx/animate.ts 做**结构级扫描**（导出面/原语拆分/重复探测）判定已良好：config 是订阅-合并-归一化函数集、voice-button 单组件组合、animate 是自带 tween/timeline 零依赖库。**至此 asr-voice 五轮优化系列收口**：v0.2.2（云端断流三修）/v0.2.3（TTS 打断窗口+超时）/v0.2.4（QueueRunner 重构）/v0.2.5（realtime-host 去重）/v0.2.6（recorder 收尾去重），累计 5 个真 bug + 4 轮重构。**诚实标准**：config/voice-button/animate 三面只做了结构级扫描未逐行读——预期价值低（纯函数/纯 UI 组合、历经 alpha 适配轮的 typecheck 与 168 测试），逐行读的成本收益比不划算；若未来要再动，从这三面的全文精读开始。可复现?是（本轮零 commit，扫描记录即证据）。
- **dsh-asr-voice 四轮：recorder.ts 收尾路径单一来源化（2026-09-02，重构/可读性）**：问题=继续重构，recorder.ts 换角度复扫。结论=08-30 的 P0 修复全在位、状态机正确，无功能性 bug；但 `for (const t of stream!.getTracks()) t.stop()` 逐字重复 **9 处**（含 6 处非空断言）、原始录音诊断上传 fetch 块重复 2 处。解法=抽 `stopStream()`（闭包持 stream，onstop/abort/错误各分支共用，非空断言清零）+ `captureDiagnostic(blob)`；**授权挂起期对新流 s 的两处停止刻意保留**——彼时新流尚未赋给 stream，走 stopStream 会停错流。验证=typecheck + 168 测试 + build 全绿、伞校验 20/20、发版 v0.2.6。坑=行数持平（656→659）但断言与重复路径清零——重构收益的度量要选对指标（安全收尾点数量/断言数），不是行数。可复现?是（本 commit）。
- **dsh-asr-voice 三轮：speech-out 双 sink 排队状态机单一来源化（QueueRunner）——重构轮的安全边界与语义坑（2026-09-02，重构/可读性）**：问题=用户点名「可读性/降耦合/优雅性/能精简则精简，不影响功能」。解法=①先勘测试安全边界：browser sink 有 4 例行为钉（排队一次一句/drain 每轮触发/cancel 吞 drain/看门狗推进）+ cloud sink 无直测——重构骨架以 browser 钉为验收；②抽 `createQueueRunner`（队列/一次一句/pending-drain/打断作废，token 序号防迟到 settle 抢跑），两条 SpeakSink 退化为「播一句（Promise settle）+ interrupt 止声」薄适配，drain 回归类 bug（历史上真实发生过）此后单点修复；顺带修一个旧潜在卡点：sync speak 抛错现在结算并推进队列（旧实现停在原句）。③realtime.ts 两处逐字重复的计时器清理助手合并 clearTimer。**语义坑（重构才暴露的时序差异）**：旧实现的 fake onend 同步调起会让队列**同步**推进，promise 化后推进落微任务——两例行为钉的同步断言红；真实浏览器 onend 本就异步，测试断言前 `await Promise.resolve()` 让一拍即等价（不改语义只对齐时序）。**本文件已记录的教训链再 +1**：改写带条件守卫的代码块又差点丢 `data.ok/data.audio` 校验（本轮靠重构前先全文重读原函数防住）——重构守卫链一律先抄全再删。**验证**=typecheck + 168 测试（19 speech-out 钉全绿）+ build；伞校验 20/20；发版 v0.2.4（bump 前 `git tag -l` 确认未占用）；服务端实吐 bundle 含 QueueRunner、rev 变化（重建即生效）。可复现?是（772d7e0）。**机制细化修正**：v0.2.5 只动 host 半区（realtime-host.ts→lib/index.js），实测 rev 不变、client bundle 无新符号——**「重建即生效」仅适用 client 半区（聚合路由按请求读 client.js）；host 半区在进程启动时装载，需重启生效**（本版为纯内部重构零行为变化，随下次重启自然生效，无急迫性）。
