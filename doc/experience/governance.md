# 治理 / 决策 / 记录（110 条）
- **dsh-model-selector 测试门禁加固：mtime 检查双向是假的 / 文本扫描一行注释即绕过 / rolldown 会 tree-shake 注入标记 / validator 只扫顶层（2026-09-17，治理/门禁/CI）**：问题=审查发现本仓库 111 项测试**在任何 CI 里都不执行**（publish.yml 只 checkout→查重→publish，伞仓库校验脚本也不装依赖），且几条关键门禁本身可被绕过或恒真。① **mtime 新鲜度检查双向都是假的**：`oldestDts >= newestClientSrc` 在 `git clone` 后**必然 FAIL**（git 按路径字典序写盘，`lib/` 早于 `src/`），开发者工作树里恒 PASS；`touch lib/types/client/*.d.ts` 又能刷绿；且完全没覆盖真正被浏览器执行的 `lib/client.js`。② **座位遮蔽守卫扫源码文本**：把 priority 改成 0 并在上一行加 `// 历史上曾用 priority: -1` 即通过（实测）。③ **产物跟踪无门禁**：`actions/checkout` 只发布已跟踪文件，而 build 出的新 `.d.ts` 没有任何测试会发现（`git diff --exit-code` 也看不见未跟踪文件）。④ **「single→chain」那一半从未被检查**（测试文档却声称有）：实际断言只有「官方产物含座位名」「产物无 `priority:`」。⑤ **validator 的假通过**：只扫顶层（无缩进）`- id:`、完全不扫 `disabled: true` → `- update:` → `- id: ui-model-selection` → `disabled: true`（缩进嵌套）判 PASS；权限等级正则 `/low|medium|high|unknown|权限|permissions?|access/i` 只要 README 出现「权限」二字即 PASS。解法=①**内容指纹**：`tsdown.config.ts` 算 `src/client/**/*.ts(x)` 的 sha256 注入产物，测试比对产物里的指纹与现算值；**关键实测**：`define` 只做字符串替换（源码不引用就不出现），而 `intro` 里的裸 `var` 与单行注释会被 rolldown **tree-shake**（8 个变体实测：`module.X=`/`globalThis.X=` 能活但分别污染导出对象与 window）→ 最终挂在 **footer 注释**里（banner/intro 与官方契约逐字一致、零运行时足迹）；`.d.ts` 侧无法注入常量，改断言「`lib/types/client` 的 .d.ts 集合 ↔ `src/client` 源文件集合一一对应」，内容一致性交给 CI。②**座位守卫改值级**：提 `export const SEAT_PRIORITY`，测试剥注释后取值断言、断言注册处写常量而非裸字面量、用花括号配平截取 `slots.register({…})` 对象（不再 `slice(start)` 到文件尾），并从产物里配平取出那个注册对象断言 `priority: -1`。③**新增 lib 跟踪门禁**（`git ls-files lib` ↔ `git ls-files --others --exclude-standard -- lib`，非 git 环境 `t.skip` 不假红）。④官方侧改读 `ui-conversation` 的 `slots.d.ts` 断言 `kind: 'single'` + `scope: 'session'`，并把 `priority:` 检查**收窄到该座位注册对象**（全量匹配会在官方别处用 priority 时假红）。⑤validator：全文件扫 `disabled: true` 并回溯所属 `- id:`（顶层/insert/update 一律纳入）、**排除自有条目**（`dsh-` 前缀不能再当官方判据——本伞自有 id 就是 `dsh-*` 形态）、权限检查必须出现等级词、新增 README 名称格式（中文名（English Name））与 `validate:registry` 检查——后者**只在 README 声明商城分发意图时 FAIL，否则 WARN**（本文件已记「validate:registry 属商城侧工具、未上架不适用」，一刀切 FAIL 会让另两个未上架仓库的 CI 持续开 Issue）。⑥**新增 CI**（push/PR：install → typecheck → test → build → `git diff --exit-code -- lib` + 未跟踪产物检查）；publish.yml 加同样前置 + CHANGELOG 段落存在性 + 发布后打 tag（**判重必须用 `git ls-remote --tags origin`**：浅克隆下 `git tag -l` 看不到远端已有 tag，会把重复 tag 当新 tag 去 push）。**坑**=①`git diff` 看不见未跟踪文件，产物门禁必须双向查；②rolldown 会 tree-shake 未被引用的 intro var 与注释——要注入标记就得挂在必然保留的位置（footer/banner）；③mtime 在 git checkout 后不可信（写盘顺序决定，不是构建顺序）；④`pnpm build` 会重写 `lib/*.map`，CI 里 build 后必须 diff **全部** lib 产物；⑤validator 的官方前缀含 `dsh-` 会把本伞自有 id 误判成官方。**验证**=111 测试全绿；变异法逐条验证（改常量值/注册处写裸字面量/注释绕过/源码改了不重建/新增未跟踪产物/删已跟踪产物/官方产物注入 priority/官方 kind 改 chain/deny-list 逐条注入/非 git 目录 skip）；伞 validator 对本仓库 22/22、对 dsh-notify 与 dsh-asr-voice 各 21/22（唯一 WARN 为「未声明商城分发意图」，零误伤）。可复现?是。
- **发版前必须验「tag 要指的内容」==「npm 已发布的包」：npm 有版本而 git 无 commit/tag 的漂移（2026-09-14，治理/发版）**：问题=四个插件仓库在 npm 上已有 0.1.12/0.1.18/0.3.1/0.4.1，但 **git 里没有对应 commit 与 tag**，四仓库各自停在上一版（npm 化那一轮只发布了、没提交）。直接补打同名 tag 有实质风险：**万一工作区内容与当时发布的内容不同，tag 就会指向一个从未发布过的东西**，而 npm 同版本无法重发、事后无法修正。解法=发版前下载 npm 上已发布的 tarball，**逐文件与工作区 `cmp`**（含 `lib/` 构建产物、`LICENSE`、图片资源）：本次 notify/selector/asr-voice/computer-use 分别为 19/32/59/297 个文件，**全部逐字节一致** → 确认可以直接提交＋打同名 tag，无需也无法重发。**坑**=①对比脚本里 `find . -type f` 给出的是相对 `$work/package` 的路径，`cmp` 时忘了拼 `$work` 前缀会把**每一个文件**都报成 DIFFERS（本次 19/19、32/32、59/59 全红）——**全红的对比结果先怀疑脚本，别怀疑内容**；②`gh release view` 与 `api.github.com` 偶发 `EOF`，重试或改用 `curl` 即可，别误判成 Release 没建成功；③tag 类型要先查（`git cat-file -t <tag>`）——本次四仓库轻量/注解混杂，统一改用注解 tag。可复现?是（任何「npm 已发布而 git 未提交」的仓库都可如此验证）。

- **AGENTS.md 触顶 65536 注入预算被截断：按性质分流把 87% 的历史挪出（2026-09-14，治理/上下文）**：问题=往 AGENTS.md 写记录后涨到 68381 字节，harness 报 `truncated AGENTS.md from 67925 to 65244 bytes`——**超出 65536 字节的 workspace 注入预算即从文件末尾截断**，最旧的条目静默消失（不报错、文件本身没坏，但旧记录不再进上下文）。原因=量化后一眼看出问题在**结构**而不是「又写多了」：规范区（每会话必须遵守）只占 12%（7273 字节），**版本升级记录占 87%（52077 字节）**；而历史里混着两种东西——**仍然生效的硬约束**（pnpm 陷阱、改名四处契约、sessionHeader 补丁仍必需）漏了会犯错，**纯过程叙事**只在考古时有用，两者却以同样体积常驻。解法=按性质分流：①规范区原文保留；②新增「关键陷阱速查（务必先读）」，把散在各轮记录里**仍生效**的硬约束提炼成条目（依赖与 lockfile／插件加载契约／LLM 与 settings／升级与发版流程／已知噪声／本文件自身）；③版本升级记录压成 12 行索引表（日期·主题·关键结论·链接）；④全部历史正文按轮次移入 `doc/history/<日期>-<主题>.md`，**原文未改**。结果 59851 → **15104 字节（-74%）**，余量 50432。**坑**=①**搬迁必须逐行校验无损**：归档是「搬家」不是「重写」，本次逐行比对「原文 247 个非空行 → 归档 247 个非空行」；第一次校验报 `LOSSLESS: False`，排查后是**我的校验脚本自己多跳了一行标题**——**校验脚本报差异时先怀疑脚本**；②**AGENTS.md 在伞目录，不在任何 git 仓库内**（`dsh-plugins/` 与各插件目录才是仓库），改它**没有版本控制兜底**，动手前必须先 `cp` 备份。可复现?是（任何超 65536 字节的 AGENTS.md 都会被截断，`wc -c` 即可复验）。
- **dsh-asr-voice 发版 0.2.11：会话中断遗留未提交工作，「版本没更新」诊断法（2026-09-05，治理/工作流）**：问题=用户问「看看最新的，为什么没有更新啊」——查 npm dist-tags（latest/next 均 0.1.2-rc.1）、全局安装、profile、deepseek-harness 源码仓全部已是最新，唯一「没更新」的是 dsh-asr-voice 的 git 历史：停在 v0.2.10，但工作区躺着一整批**已完成且 193/193 测试通过**的改动（31 文件 +519/-207：SSE 背压单槽 coalesce 改有界保序队列——背压中连续两句收口时第一句 final 被第二句顶掉、客户端永远只看到 partial 缺句；半双工静音态探针不死判——setMuted 全零帧被误判设备静音杀掉短句会话；withTimeout settled 竞态防护；connectTimer 清理；providerView 归一化；tween delay 仅首帧），另有两个未跟踪新测试（http/presets）。
原因=上一会话在 typecheck 前被截断，后续真正完成的工作没走到 commit；「版本/依赖已对齐」与「工作已落盘」是两回事，只看 git log/tag/版本号会漏掉工作区状态。
解法=①诊断顺序：npm dist-tags → 全局/链接安装 → 源码仓 → 各插件仓 `git status`（对照 git log），一次看清「什么没更新」；②发布路径：`pnpm build`（lib 与 src 一致）→ typecheck + `node --test` 全绿 → 功能 commit（含未跟踪新测试，`git add -A`）→ bump version → `chore: 发版 0.2.11（…）` commit → `git tag v0.2.11`（先 `git tag -l` 确认未占用）→ push main + tag。
坑=①**「已完成且全绿」的工作不等于已提交**——会话截断后 git 历史与工作区会分叉，查「为什么没更新」必须看工作树；②lib/ 产物是跟踪文件，改 src 后必须 build 再 commit，否则发布态缺新代码（check-artifact-imports 门禁的孪生教训）；③伞契约经验落档是任务完成的一部分，不能省。
验证=typecheck ✅、193/193 测试 ✅、push main（678bd7f..29908c0）+ tag v0.2.11 ✅。可复现?是（截断会话后 `git status` 与 `git log` 对照即可复验）。
