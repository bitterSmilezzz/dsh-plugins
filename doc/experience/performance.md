# 性能 / 资源 / 优化（63 条）
- **dsh-model-selector 二轮审查 5 项低优全落地：null-ref 换入跳过官方 hook、拖动监听动态挂载、幂等判据纯函数化（2026-09-06，性能/优化 + 修复）**：
问题=首轮审查（b9ce4ff，见 fixes.md 同日条）后继续清低优遗留：①`useAnchoredMaxHeight`（官方 primitives，自带未节流 capture-scroll+resize fit，每事件 getBoundingClientRect+setState）与插件自建 rAF 合并 measure 双套监听并存，且**向下弹（menuAbove=false）时 hook 返回值在 maxHeight 三目处被忽略**——监听纯属白付测量；②`useEffortDrag` 的 window 捕获 move/up/cancel 三监听**菜单整个打开期间常驻**，不拖动也挂着；③四个调优常量缺「调法」说明；④locales.ts tab/空格混用；⑤「cancel 后平台补发 pointerup 的幂等清理」判据只在 hook 内联、无测试。
解法=①**null-ref 换入**（React hooks 规则下「条件跳过 hook」的标准姿势）：hook 不能条件调用，但被测 ref 可以条件替换——`useAnchoredMaxHeight(menuAbove ? menuRef : inactiveMenuRef, …)`，向下弹期间传入恒 null 的空 ref，hook 的 layout effect 因 `el === null` 整体跳过（不注册监听、不测量）；方向翻回向上时 ref 身份变化使 effect 重跑并立即 fit——**顺带修掉原实现的陈旧钳位**（signal=open 不变时 hook 不重跑，翻回向上用的还是向下弹期间按「顶边固定布局」量出的旧底缘值）；②监听 effect 改 `[dragging]` 依赖：`if (!dragging) return` 后注册、拖动结束卸载，refs 间接层（globalMoveRef 等每渲染重赋值）保留使回调恒最新；**语义等价性论证**：pointercancel 终态后补发的 pointerup 要么因监听已卸载收不到、要么走 `dmsIsActiveDrag` 守卫幂等跳过，可观察行为与常驻挂载一致；pointerdown→effect 落地缝隙内指针仍在输入条上，由输入条自身 JSX 事件兜住，无事件丢失；③每个常量注释补一行「调法：调大/调小分别得到什么」，README 开发节加一句指向；④locales.ts 两词典统一一个 tab 缩进（词典键解析正则 `^\s*"…":` 与缩进无关，纯格式零风险）；⑤抽 `dmsIsActiveDrag(pointerActive, activePointerId, pointerId?)` 到 effort.ts：pointerActive=false 或 pointerId 与活动指针不一致 → 跳过终止动作（不清状态、不回调）。**stop/cancel 双路径统一走它**——cancelDragging 原判据 `activePointerId !== pointerId` 与「pointerActive ⟺ activePointerId!==null」不变量下等价（两处清态永远成对），逐条核对后才合并；+3 单测（非活动期跳过 / 异指针跳过 / 匹配与 blur 兜底放行）。
坑=①**先 build 再跑测试**（老教训复验）：`plugin-contract.test.mjs` 的「client d.ts not stale」是 **mtime 门禁**（oldestDts ≥ newestClientSrc），改完 src 直接跑 test 必红，build 后全绿——验证链顺序 typecheck → build → test；②**增量构建只重写内容变化的 d.ts**：ModelSelect.tsx 只改内部实现、导出签名未变，`lib/types/client/ModelSelect.d.ts` 内容零变化没被重写（.map 重写了），门禁靠目录内其它新鲜产物过——判断「d.ts 没进 diff」不是构建失败；③**抽纯函数先证等价再合并**：两个守卫（stop 的双条件 / cancel 的单条件）形态不同，靠「两处清态成对 ⇒ 不变量成立」推导等价，不能凭直觉合并；④hooks 不能条件调用，但「跳过 hook 的工作」可以靠换入空 ref / 换 signal 达成——这是不动官方包的前提下的标准做法（改官方 primitives 违反 DSH-Store 门禁）。
验证=`./node_modules/.bin/tsc -p tsconfig.host.json --noEmit` + client 双零错；`tsc host + tsdown + tsc client.build` 全成功；`node --test test/*.test.mjs` **64/64**（61 存量 + 3 新增，全是此前确认的基线+新增）；`git diff --check` 零空白问题；commit 55e0151 已本地提交未 push，版本号未动，lib/ 产物与 src 同批入库。可复现?是（`cd dsh-model-selector && node --test test/*.test.mjs`；向下弹场景开菜单后查 window 监听数/`scroll` 捕获监听注册即证 hook 已跳过；拖动开始前后 `getEventListeners(window)` 可见三监听动态增删）。
- **agent-qa LLM judge 有三类确定性盲区：toast 瞬时、原生 title、无法 resize/滚动——探针分工补齐（2026-09-01，测试方法论 + 修复）**：
问题=dsh-model-selector 完整 8 条 agent-qa 真机用例首跑 5 过 3 挂，三条失败**都不是产品 bug**（插件行为由瞬时探针逐一证明正确），而是 agent-qa 的 LLM judge 能力边界：①**toast 瞬时**（`10-switch-announced`）：官方 Toast 是 hold 3s + fade 1s 的 body portal `role="alert"`，而 judge 每步「截图 + LLM 判定」要 30–90s，永远错过窗口；②**原生 title 不可见**（`50-reasoning-badge`）：推理徽章的提示放在 `title` 属性，headless 截图与 ARIA 树都不渲染它，hover 也不弹；③**无 resize 工具 + 无可滚动内容**（`60-menu-direction`）：judge 明确「没有 resize window 动作」，且新会话 docScrollable=0，无法把 trigger 滚到视口顶部来复现「向下弹」。
解法=①**探针分工**：新建 `checks/toast-probe.mjs`（点选推理模型后 50ms 采样 body 4.2s，断言 `body > [role=alert]` 弹出且文案 = 自动拉档 toast）证明 toast 真实存在；`dom-probe.mjs` 增 badge title 非空断言与短视口(400px)「菜单在视口内 + 列表可滚动」断言；②**方向决策抽纯函数**：把 `setMenuAbove(rect.top >= innerHeight - rect.bottom)` 与 `belowMaxHeight` 钳位抽到 `src/client/menuFit.ts`（沿用 effort.ts 的「纯逻辑搬出 .tsx 供 node --test」模式），`test/menu-fit.test.mjs` 补 5 例单测（上大向下弹 / 下方更大向下弹 / 矮窗口不再要求上方空出满高的回归防护 / 钳位边界），总单测 34→39；③**agent-qa 用例只留 judge 能验证的持久交互流**：10 去掉 toast 步骤（toast 归 toast-probe）、50 去掉 hover-title 步骤（title 归 dom-probe）、60 只留「菜单在视口内」（方向翻转归 menuFit 单测 + dom-probe 短视口）。重跑 10/50/60 三条全 PASS，8 条全绿；dom-probe 含新断言 PASS；axe 无新增违规。
坑=①**不要用浏览器 stub 伪造布局**：曾试过覆写 trigger 的 `getBoundingClientRect` 强制向下弹——方向决策确实翻转（menuBelow=true），但菜单仍按真实位置布局（top-anchored 到真 trigger 下方），`inside:false` 制造出「人为的不真实失败」，改成纯函数单测才干净；②**矮视口不必然触发向下弹**：新会话 trigger 恒在视口 ~63% 处（bottom-fixed 布局），400px 甚至 200px 视口下「上方空间仍大于下方」，向上弹始终是正确决策——想用 resize 复现向下弹是错误前提；③**首跑能过不代表可复现**：60 的「滚动到顶部」首轮 PASS 是因为当时会话有历史内容可滚，重跑换新会话 docScrollable=0 就翻车——凡依赖会话内容的步骤都不可靠，要么固定会话状态要么下沉到确定性探针；④**spacer 注入也拉不动**：在 body 前插 1200px 空 div 让文档可滚，但 composer 是 bottom-fixed，trigger 仍钉在原位。
验证=主仓 39 单测全绿 + typecheck + build；QA 工作区 dom-probe / toast-probe / axe-probe 全 PASS；agent-qa 8 条全绿（10/50/60 重跑确认）。可复现?是（`checks/toast-probe.mjs` 必抓到 toast；矮视口下 trigger 恒在中下部；新会话 docScrollable=0 必现——三条都是稳定可复现的环境事实）。
- **门禁要有牙：13 个变异把 5 条新断言逐个证伪，抓出「子串匹配 CSS 类名」假通过（2026-08-31，性能/优化 + 测试）**：
问题=0.1.7 修完，测试面从 17 例扩到 34 例（类名/词典门禁 4→9、档位纯函数 +6 轮随机化、新增产物与清单 6 例），全绿。但「全绿」不是证据：一条永远为真的断言与一条真在守护的断言，在终端里长得一模一样。本仓此前已两次被解析器欺骗（见上一条），所以这轮专门用来攻击自己刚写的门禁。
解法=①**每条新断言配一个可执行的假样本**：类名门禁加 4 条 self-test（注释里的 `.dms-retired` 不得算声明、模板洞与字符串拼接必须取到词、`styled∖used` 与 `used∖styled` 两个方向都能报出人造差异、空词典必须解析成空集以便「抽取失效」能被发现）；②**纯函数升级为随机化轮**：`test/effort-fuzz.test.mjs` 用 mulberry32 定种生成 4000× 结构合法但形状刁钻的 `ModelDirectoryState`（current 指向加载失败的供应商、`reasoningEffort` 是切模型后残留的野值、adapter 报非规范档位 id、0/1/7 档、defaultEffort 缺失），断言滑杆永不会拿到越界/非整数 index、`index/(len-1)` 恒有限（它直接乘画布宽度）、降级链严格「用户选 → 模型默认 → 中间档」、`dmsCurrentModel` 只会返回快照里真实存在的模型对象（引用比对）、以及**纯函数不得改写快照**（JSON 前后比对——这些都在 render 里跑）；③**产物与清单钉住**：`test/package-meta.test.mjs` 断言 loader id == 包名、运行时 external 恰好是 `react`/`react/jsx-runtime`/`@deepseek-ai/dsh-client-ui-primitives`（多一个官方模块就必须过一次准入讨论）、26 个已退役类名在源码与产物里都不得复活、README 承诺的「无自有网络/无文件/暂无持久化」用产物里 `fetch(`/`XMLHttpRequest`/`localStorage`/`require("node:` 零命中来兜。
坑=①**`bundle.includes(className)` 判「样式真的进了产物」是假通过**：类名同样出现在编译后的 `className` 字面量里，把产物 CSS 中 `.dms-badge` 改名成 `.dms-badgeXX` 测试依然绿——改成只抽产物里的 `const CSS = \`…\`` 载荷，并用 `(?<![\w-])name(?![\w-])` 词级匹配（顺带解决 `dms-model-row` 误命中 `dms-model-row-effort`、`dms-check` 误命中 `dms-model-check`）；②**变异要先确认「改动真的落盘」**：首轮 `min→max` 报「未捕获」，实际是 perl 括号未转义导致文件根本没被改；给 `run_mut` 加了 `cmp` 前置判定输出 `NOT-APPLIED`，否则假结论比没测更糟；③**变异可能被「错误的断言」抓住**：我往产物头部插 `fetch(...)` 时先撞坏了 `^window.__ModuleLoader__` 锚点，`loadable` 那条先红——看似已验证权限围栏，其实权限围栏没跑；换成不破坏锚点的插入点后才是它真的报错；④随机化轮的循环变量初始化写歪一次（`for (let n = rand(); ...)` 把更新位写成三元空表达式），能跑但不可读，改成普通计数循环。
验证=**13 个变异全部红**：档位 4（忽略用户选档 / 允许 1 档滑杆 / `rank >=` 破坏「全未知取首个」/ 钳位 `min→max`）、CSS 词典 4（死规则复活 / 现役类丢规则 / zh 键改名 / TSX 用了未样式化的类）、动态类围栏 1（`className={`dms-root ${extra}`}` 必须报「洞里没有字面量」）、退役名单 1（单独 `--test-name-pattern` 跑，避免被前一条测试抢先报出）、产物 3（多出未声明 external / loader id 漂移 / 产物 CSS 规则被改名或整块丢掉，另加 `fetch`+`localStorage` 两次注入）。变异全部用 `cp` 前后快照恢复，跑完 `shasum -a 256 lib/client.js` 与提交态一致（构建本身两次哈希相同＝确定性）。收口：`pnpm typecheck` 双 program 绿、`node --test` **34 例全绿**、`pnpm build` + `node --check` OK、`validate-plugin.mjs` 19/20（唯一提示仍是 0.1.7 未打 tag）。可复现?是（`cd dsh-model-selector && node --test test/*.test.mjs`；随机化轮打印 seed，失败可精确重放；把任一断言换成常量 true 会立刻被对应变异揭穿）。
- **model-selector 死 CSS 清理真正做成：类名 diff 升级成双向门禁（2026-08-31，性能/优化 + 契约细节）**：
问题=dsh-model-selector 从两层级菜单单层化后，`styles.ts` 同时存在两类静默失衡：①**TSX 在用、CSS 零规则**（选中行 `dms-model-optionSelected`、搜索结果里的供应商标 `dms-model-option-provider`——后者正是历史上为「跨供应商同名模型」专门加的元素，等于白渲染）；②**CSS 声明、无人使用**的死选择器 26 个（约 141 行）。`pnpm typecheck` 与 `build` 全绿也发现不了——CSS 类名对 TS 只是普通字符串。本文件前两轮曾判「静态 diff 不可靠、无视觉回归基线就不删」而放弃。
原因=旧结论的前提是**抓取面不全**，不是静态 diff 本身不可用：①CSS 注释里写着的旧类名（`/* 旧规则 .dms-option.dms-selected 已成死码 */`）被算进「声明面」，必须先剥 `/*…*/`；②模板拼接类名 `` `dms-effort${dragging ? " is-dragging" : ""}` `` 按原样切词会得到 `dms-effort${dragging` 这种非 token 残片，**真实类名反而丢失**——首轮跑出 31 个「死类」，其中 `dms-effort`/`dms-model-option`/`dms-chevron` 全是误报；③必须先证明不存在变量拼类名：全文件 5 处 `className={` 拼接全是源码字面量、零 `dms-${` 插值，静态集合才等于运行时集合。
解法=①建 `test/classnames.test.mjs` 做**双向**断言（`styled∖used == []` 且 `used∖styled == []`，只统计自有命名空间 `dms-*`/`is-*`），并钉住词典三面：zh/en 键集相等、每个 `t("x.y")` 有定义、每个键都被引用——这四条把「重构顺手丢了样式/丢了文案」这一整类回归永久拦在测试里；解析按 `className=` 起点做花括号配对扫描，`${}` 先单独递归取词、再把挖空的静态文本按空白切 token。②删 26 个死选择器，其中两处按**改名救活**而不是删：`.dms-check svg{display:block}`（消除 svg 基线浮高）改名到现役 `.dms-model-check svg`；`@keyframes dms-menu-in` 的入场动画与 `prefers-reduced-motion` 关闭分支从 `.dms-model-menu` 一并迁到 `.dms-menu`（否则动画键帧成孤儿、菜单入场过渡彻底消失）。③合并 `.dms-model-option` 的两处重复定义（原先与已删的 `.dms-model-row`/`.dms-model-back` 共享一条基础声明）。
坑=①**`useAnchoredMaxHeight` 只适配「底边固定、向上生长」的浮层**：它内部读浮层自身 `getBoundingClientRect().bottom - MARGIN`（MARGIN=12 未导出，本地要写常量对齐）。向上弹时面板底边由 `bottom: calc(100% + 8px)` 钉住、与高度无关，读它正确；**向下弹时面板顶边固定**，同一个值退化成 `当前高度 - 12` → 每次 resize/scroll 再缩 12px 的自我反馈。修法是分方向：向上用官方 hook，向下用 trigger 实测 `innerHeight - rect.bottom - 12`（trigger 高度与本浮层无关，无反馈），且本地 effect 同样要监听 `resize` + **capture 阶段 `scroll`** 才与 hook 的重测时机对齐。②**相似名不等于同物**：`.dms-check` 与在用的 `.dms-model-check` 是改名遗留而非两个控件——早先据此判「需运行时验证」而放弃，正确动作是改名救活。③删 CSS 只能按门禁输出逐条核对，不能按行号整块删：`.dms-model-row, .dms-model-option, .dms-model-back {…}` 这类共享选择器列表必须只摘死项、把活项声明合并留下。④我自己还犯了一次「删块改成复制块」（`old_string` 只锚到下一块首行），**删类改动完立刻重跑门禁**，两个方向都能暴露。
验证=`pnpm typecheck` 双 program 通过；`pnpm test` **17 例全绿**（effort 纯函数 13 + 门禁 4，其中三条门禁由红转绿）；`pnpm build` 后 `node --check lib/client.js` OK，产物里 8 个旧类名除 1 处注释外**零命中**，`require("@deepseek-ai/dsh-client-ui-primitives")` 保持 external 未被 inline；`styles.ts` 709→568 行；`validate-plugin.mjs` 19/20（唯一提示＝版本 0.1.7 尚未打 tag，发版时对齐）。**未做**：浅色/深色与菜单翻转方向的浏览器实测（3080 由用户手动启停）。可复现?是（`cd dsh-model-selector && pnpm test` 直接重跑双向门禁；把任一 `.dms-model-*` 规则改名成不存在的类名，会立刻红在 `used∖styled`）。
- **优化循环第 5–6 轮：装载验证抓出「profile 级启动阻断」+ 第二个 secret 字段（2026-08-30，性能/优化）**：
问题=六仓已过静态门禁且 148 例测试全绿，但「能用」仍缺端到端证据——dsh-email 从未真正装载过（不在 web profile 里，只跑过 build+test）。
原因=静态检查与单元测试都不覆盖 **DSH 装配期约束**：`package.json` 一旦声明 `dsh.client`，client-modules 就要求 `lib/client.js` 真实可加载，否则**整个 profile 起不来**（不是只坏这个插件）。dsh-email 是纯 host 插件（上游 npm 里的 `lib/client.js` 本就是 0 字节占位），fork 丢了占位文件却保留声明，且 inject 了 alpha.2 已不存在的 `@deepseek-ai/dsh-client-runtime`。
解法=①建一次性 profile（只装 base + web-app + 待验插件，`.npmrc`/`pnpm-workspace.yaml` 沿用，`pnpm install` 后 `dsh --profile emailcheck --port 3097 --no-open`）——**立即复现启动失败**，报错精确指向缺失的 client bundle；删掉 `dsh.client` 声明后启动零错误、`/_dsh/dsh-email/settings` 真机返回 200、伪造跨站 Origin 返回 403。②把这类"声明与产物必须自洽"固化成两条门禁：`check-artifact-imports.mjs`（相对 import 完整性）+ `validate-plugin.mjs` 第 20 项 `client.bundle`（声明 dsh.client 必须有非空 lib/client.js，反向情形 WARN），并用真实缺陷 tag email v0.8.3 做反向验证（跑出 FAIL）、用 6 个新 tag 跑正向（全过）。③装载响应里发现 `hasPassword: true` → 本机**确实存着真实授权码**，即修复前该凭据可经 HTTP 明文读走；顺带查出掩码只覆盖 `password`，漏了同为 `role('secret')` 的 `accountsYaml`（多账号 YAML 内嵌每个账号授权码）→ 掩码/哨兵泛化到两个字段并补 2 例（email 76→78）。④测试面从 4 仓扩到 **6 仓全覆盖**：shortcuts 用最小 `window`/`CustomEvent` 垫片直跑 `src/client/config.ts`（7 例，钉住「广播传浅拷贝」「写失败回滚+派发 config-error」「dispose 后写路径置空」——都是 8/28 那轮修复却零测试守护的点）；model-selector 把 `EFFORT_RANK` 与 6 个档位纯函数从 `.tsx` 提到 `src/client/effort.ts`（node 无法剥离 .tsx 类型，留在组件文件里就永远测不到），补 13 例覆盖钳位边界、三层降级、非推理模型不抛异常。
坑=①**`.mjs` 里不能写 TS 注解**——node 的类型剥离只对 `.ts` 生效，我把 `const dispatched: Array<{...}>` 写进 `.mjs` 直接 SyntaxError；②**按行号删代码会因同会话内的前序编辑而错位**——我按早先 Read 的行号删，结果把保留段删掉、目标段留下（typecheck 报「Import declaration conflicts with local declaration」才暴露），必须改用「按名字 + 花括号配对」并**先干跑打印范围**；③**向前吞注释的循环要写死回溯上界**，否则 `j` 收敛到 0 后不再前进会死循环（我这条命令因此挂到超时被后台化，好在写盘在循环之后、文件未受损，`git checkout` 一发即恢复）；④**bump 版本前必须查现有 tag**：我给 model-selector 二次 bump 成 0.1.5，而 0.1.5 已被上一条修复的 tag 占用 → `git tag` 直接 fatal；修法是前进到 0.1.6，不移动已发布的 tag；⑤子代理给的"死 CSS"结论不可照删：换成允许连字符 + 计入模板拼接前缀（`` `dms-effort${x}` ``、`` `dms-chevron${open?...}` ``）后，未引用类数从 11 变 27，而条件态（error/loading）也不会被一次 DOM 采样覆盖——没有视觉回归基线就不删，几 KB 收益不值（**2026-08-31 已推翻**：前提只是抓取面不全，补齐后删掉 26 个死选择器／141 行，见本文件顶部条目）；⑥我第 1 轮的「retry 与 asr 围栏已分叉」是错的，13 条同源夹具两仓全绿即证一致。
验证=六仓 `dirty 0 / ahead 0 / version == 最新 tag`，门禁 20 项 5 仓全绿 + email 19/20（唯一提示为正确的「7 工具需专项评审」），测试合计 **148 例全绿**（email 78 / asr 28 / ms 13 / retry 13 / notify 9 / shortcuts 7）；一次性 profile 与备用端口实例用完即停即删，3080 由用户实例承载最新构建。可复现?是（clone email v0.8.3 建 profile 启动必失败；`node --test` 各仓可直接跑）。

- **插件优化测试-收敛循环第 2–4 轮（2026-08-30，性能/优化，落地 8 项 + 3 项评估放弃）**：问题=alpha.2 迁移与账号收编完成后继续找可优化项，要求「测试-优化循环直到收敛」，每轮完成即推远端。
解法（按仓落地）——**model-selector**：①两条 `file:../../dsh-upgrade/...` 本地路径依赖换成已发布 `^0.1.2-alpha.2`（此前别人 clone 无法安装依赖），升线暴露 4 处契约变化并修：`ModelReasoning/ModelSelection/ModelProviderGroup` 改从 `@deepseek-ai/dsh-api-session-controller/types` 导入（官方自身即此路径）、`ctx.slots`/`ctx.sessions` 补 renderer/client 与 session-controller/client 增补、slot `inject` 回调参数 `SessionId → string`（alpha.2 放宽，声明更窄会因逆变报错，branded API 处收口转换）、`reasoning.efforts` 变 readonly → **把 helper 参数一并改 readonly 而不是拷贝数组**；**notify**：②深链基址由 apply 时一次性快照改为惰性求值（fiber 早于 webServer 就绪、或 `--port` 起非默认端口时快照会永久停在 3080 回落值、点击跳转静默失效）；③`summaryOf`/`isSubagent` 提为 `src/notify-policy.ts` 纯函数并补 9 例；**retry**：④未配置时显示 host 下发的 `defaultMaxRetries`，不再硬编码 `'5'`（双源）；⑤`isTrusted` 导出以便夹具钉住；**email**：⑥Origin 围栏 + 授权码掩码 + 空串哨兵（见同日安全条）；**跨仓**：⑦`validate-plugin.mjs` 两处假阳性修（YAML 注释计入 entry、src+lib 双扫把 7 工具报成 14 把 WARN 升 FAIL）；⑧删除 notify/shortcuts/model-selector 三仓 package.json 的僵尸 `pnpm.overrides`（pnpm 11 只 WARN 忽略，形似钉版实则失效；删后 install+typecheck+build+test 全绿）。
评估后放弃 3 项（风险高于收益）：a) **shortcuts 热键表单一来源化**——`SHORTCUTS` 表只供帮助面板渲染、真实匹配是 13 个硬编码 if（新增键要改两处），但对照代码逐条核实**只有「多命中」无「漏命中」**（`Cmd+Shift+,` 会连带触发 settings、`Cmd+Alt+Shift+N` 触发 newChat），且分派链中间夹着 `isEditable` 早退把 12 条绑定分成「全局可用」与「输入框内不劫持」两类，改造需引入 `blockInEditor` 维度、回归面覆盖全部 12 条日常键；b) **model-selector 死 CSS 清理**——脚本比对 styles.ts 声明的 73 个 `.dms-*` 与源码引用面得 11 个未命中（`.dms-option`/`.dms-badge`/`.dms-check`/`.dms-adapt-panel` 等，单层化后的遗留），但 `.dms-check` 与在用的 `dms-model-check` 属**不同名相似形**，删除需先用运行时类集合 diff 验证，且收益仅约 90 行 CSS（client.js 63.3KB 的零头）（**2026-08-31 已做完**：`.dms-check` 属改名遗留、已改名救活，26 个死选择器／141 行删除，见本文件顶部条目）；c) **asr/notify 客户端纯函数测试**——`heuristicOptimize`/`parseHotkey` 在 client 半区被 tsdown 打成单一 `lib/client.js`，无独立可 import 模块，从 `src/*.ts` 直跑需先解 DOM 耦合。
坑=①**版本号与 tag 必须同一批**：给 model-selector 打 `v0.1.3` 时漏 bump package.json，`validate-plugin` 立刻报 WARN「manifest 一致」；修法是前进到 0.1.4 并撤销我刚建的坏 tag（`git tag -d` + `git push origin :refs/tags/vX`），不硬推、不留下坏 tag 长期腐化；②**依赖来源换 npm 后必须重新浏览器实测**，不能只看 typecheck/build 绿——`build` 只跑 host tsconfig，client 半区类型错误要靠 `typecheck`，且 inject 表里那条已消失的 `dsh-client-ui-slots` 今日实测仍能解析（不盲改，改前先证明现状可用）；③`grep -c x file | head && echo 有` 一类管道判定是假阳性（退出码来自 head）。
验证=六仓 `validate-all` 全绿（email 18/19 唯一 WARN 为正确的工具数评审）；测试面从「6 仓仅 email 有测试」变为 email 68 / retry 13 / asr 13 / notify 9 共 **103 例**、shortcuts 与 model-selector 待上述 a/c 项决定；3080 已带全部最新构建重启（`--dump-config` 与三条路由 200/200/405、日志零错误）。可复现?是（各仓 `pnpm test`、`node scripts/validate-all.mjs`）。

- **五插件深度性能优化第二轮（2026-08-28，性能/优化 5 项 + 3 项评估放弃）**：问题=第一轮收敛后用户要求继续找优化点、做完自动 push。解法（安全落地 5 项）——**asr-voice**：①optimize.ts LLM 流式输出改 `parts.push` + `join`（替代逐 chunk `output +=`）；②transcribe.ts ASR content 多模态数组形状改循环拼接（避免 map 中间数组）。**notify**：③spawnPowershell 加 onErrorCleanup，Windows toast 临时 .ps1 在 spawn 失败时兜底 unlink（正常流程脚本自删，失败残留才清）；④playSound 同类 300ms 节流（瀑布审批连续通知不叠加振荡器节点，也避免'叮叮叮'噪声）。**retry-settings**：⑤POST 逐 ns 捕获 mutate，失败聚合进响应 failed 数组，不再一个 ns 失败抛 500 掩盖其他成功（client 端部分失败显示 retryFailedSome 警告）。**共性**：给四个拆分插件 .gitignore 加 `.codegraph/`（CodeGraph 索引目录曾两次被误提交进 perf commit，软重置清理）。**评估后放弃 3 项**（复杂度/风险高于收益）：a) model-selector `choose`/`renderModelOption` useCallback 化（400 项闭包重建是 µs 级，菜单渲染频率=人类键入频率，重构引入依赖分析 bug 面）；b) config applySnapshot 序列化去重（setConfig 本地乐观更新 + host 回写确认是**两个独立信息源**，双 announce 是语义必需，低频开关操作下可接受）；c) retry-settings describe 缓存+watch 失效（GET 频率极低，克隆开销亚毫秒，加缓存增大 bug 面）。**坑**=①**用 node 脚本批量改写复杂 JSX/嵌套括号函数体极易截断文件**（choose 的 useCallback 包裹实验把 ModelSelect.tsx 从 849 行截到 667 行），必须 `git checkout` 恢复 + 用 edit 工具精确替换或直接放弃；②`git add -A` 会把 CodeGraph 自动生成的 `.codegraph/` 一起提交——提交前检查 `git status`，perf commit 混入索引目录是噪音；③Windows .ps1 的卸载清理只在 spawn error 时做，正常完成由脚本内 `Remove-Item` 自删——两路都覆盖才算完整。**验证**=五插件 typecheck 全过 + validate-all 19/19 全绿；asr-voice/notify/retry-settings 各 push 1 commit，四个拆分插件 push gitignore commit。可复现?是（远端 main HEAD 可查）。
- **五插件深度性能优化轮（2026-08-28，性能/优化 18 项）**：问题=用户要求把 dsh-asr-voice/model-selector/notify/shortcuts/retry-settings 的"资源性能、前端优化彻底做一遍，一直找优化点直到找不到"。解法（分插件）——**asr-voice（10 项）**：①animate.ts `applyNumeric` 每帧 `getComputedStyle` 读 transform（wave 无限循环动画每帧强制样式计算）→ 补间开始缓存 baseMatrix 复用；②onLevel 频谱加 0.01 阈值 + toFixed(2)（rAF 每帧只写变化 >阈值 的 CSS 变量）；③startLevelMeter 分析 AudioContext 模块级复用（不再每次录音 new/close）+ rAF 句柄保存供 stopLevelMeter 取消（onstop/onerror/abort 三路径接入）；④host 超时传播：transcribe 上游 fetch 与 optimize 的 `ctx.llm.stream` 加 `AbortSignal.timeout(60s)`（alpha.1 GenerateOptions.signal 已支持，客户端 60s 超时后 host 不再挂死）；⑤config announce 传浅拷贝（消费方不拿全局 config 可变引用）；⑥settings-card 模型目录 60s 模块级缓存（重复挂载不再每次拉 /api/asr-voice/models）；⑦频谱条 memo 化（browser 引擎 interim 变化重渲染组件时 12 柱虚拟 DOM 不再重建）；⑧wave ring `will-change: transform, opacity`（合成层）；⑨UsageStats 5s 轮询加 `document.hidden` 门控（页签隐藏暂停、visibilitychange 回来补拉）；⑩blobToWav16k 增益并入写入循环（长录音省一遍数十万采样遍历）+ hotkey `parseHotkey` 缓存（keydown 高频路径不再每键 split 字符串）。**model-selector（4 项）**：①choices 搜索索引从 useMemo 改 ref 缓存（原每次目录状态变化含 select 都全量重建 400+ 条 haystack 字符串，改仅 groups 引用变化才重建）；②搜索命中渲染上限 MAX_VISIBLE_HITS=100（宽泛关键词防 DOM 爆炸 + 提示）；③`.dms-groups` 滚动容器加 `contain: content`；④IconChevronDown/Check/Clear 从函数组件改模块级常量元素（每次渲染复用同一 React 元素）。**notify（2 项）**：①config announce 浅拷贝；②notifyMac 的 terminal-notifier 路径 `existsSync` 探测缓存（原来每次通知都探测文件系统；注意 Windows .ps1 随机名+自删是有意的安全设计不能缓存）。**shortcuts（2 项）**：①enabled=false 时动态 detach 全局 capture keydown（原来每键一次空 onKeyDown 触发）；②config announce 浅拷贝。**retry-settings**：判定核心已简单（单卡片+低频路由）无热点。**坑**=①组件内联图标改模块级常量时 JSX 用法从 `<Icon />` 改 `{Icon}`，单行三元里对象字面量歧义要括号；正则批量改写 JSX 函数体会留 `return(...)` 残余导致语法错误，改完必须 tsc 验证；②React 18 自动批处理已合并 setState，勿为合并而过度重构；③notify Windows .ps1 随机路径是防注入+防残留的安全设计，性能上不可优化。**验证**=五插件 typecheck+build+validate-all 19/19 全绿，四仓库各 2-4 个 perf commit 已 push。可复现?是（perf commit 均在 main HEAD）。
