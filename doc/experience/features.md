# 新增 / 实现 / 增强（64 条）
- **dsh-asr-voice I5+I6：真云端实时 provider（qwen3-asr-flash-realtime）与 CloudTtsSink（qwen3-tts-flash-realtime）——OpenAI Realtime 兼容面的两个方向（2026-09-01，功能/实时通道）**：需求=路线表 I5（host 实时通道换真 provider）/ I6（云 TTS PCM 播放）。实现=①**`src/realtime-dashscope.ts`**（I5 上行方向）：undici `WebSocket`（Node 24 全局，`new WebSocket(url, {headers})` 握手带 `Authorization: Bearer`，ws-auth 已证）连 `wss://dashscope.aliyuncs.com/api-ws/v1/realtime?model=qwen3-asr-flash-realtime`；建连发 `session.update`（`input_audio_format:pcm` / `sample_rate:16000` / `turn_detection:{type:'server_vad', threshold:0.0, silence_duration_ms:400}`，官方推荐 0.0/400）；`send(pcm)` 把 int16 LE 转 base64 发 `input_audio_buffer.append`；事件映射——`input_audio_buffer.speech_started/stopped`→speech-*、`conversation.item.input_audio_transcription.text`→partial（**text=已确认前缀 + stash=草稿后缀，拼接才是完整预览**）、`…completed`→final（transcript 字段）、`error`→error（code）、`session.finished`→null（本地不消费）；`close()` 必须**先发 `session.finish` 再等 `session.finished`**（VAD 模式先关连接会丢在途 final，官方警告）。②**`src/realtime-tts.ts`**（I6 下行方向）：一次请求 = 合成一整句（client 分句泵已切 ≤200 字符块，整段 PCM 返回即可，无状态比 TTS 版 RealtimeHost 简单得多）；`session.update`（`voice`/`response_format:pcm`/`sample_rate:16000`）→ 等 `session.updated` 确认再 `input_text_buffer.append`+`commit`（**手牌欠着：服务端未就绪送文本会丢**）→ `response.audio.delta`（base64）拼 Buffer → `response.audio.done` 收口（**不要等 `response.done`，部分模型不保证送达**）→ `session.finish` 收尾。③**凭据复用**：`REALTIME_PRESETS`/TTS 的 `keyPreset` 指回 `CLOUD_PRESETS` 预置 id（dashscope→`DASHSCOPE_API_KEY`），`resolveApiKey`（settings 明文 → credentials.resolve → 同名环境变量三级）天然命中——配过百炼 LLM 的用户零配置。④**host 分派**：`index.ts` 的 `createProvider` 按 `settings.realtime.provider` 分派（`''`/`builtin`→假 provider 开发态；预置 id→真云端），**无 key 时 connect 抛错让路由 502 带原因，不静默降级**（静默回退假 provider 会假装在识别）。⑤**client `createCloudTtsSink`**：与 `createSpeechSynthesisSink` 同 `SpeakSink` 接口（enqueue/active/cancel/onDrain/dispose/prime），voice-chat-button 按 `realtime.tts` 分派；`AudioBufferSourceNode` 的 `onended` 精确可靠，**不需要 utteranceWatchdogMs**（与 speechSynthesis 的不可信 onend 相反）；TTS 流式播放在 16k int16→Float32 后用 `createBuffer(1, frames, 16000)`（AudioContext 会重采样到设备率）。⑥测试 12 例：本地起**真 WS 服务**模拟 qwen 协议（服务端帧无掩码、客户端帧带掩码要解，RFC 6455 掩码 XOR），覆盖握手认证 / session.update 参数 / base64 上行字节一致 / 事件映射 / 优雅关闭 / 无 key 拒绝 / 无关事件忽略 / TTS 会话配置 / PCM 拼接收口 / error / 握手 401 / 静默断连。**坑**=①**`import type` 不能出现在 `.mjs` 测试里**（SyntaxError: Unexpected token '{'），JS 测试文件连运行时类型导入都是语法错；②**`ws.send()` 在 CONNECTING/CLOSING 态抛 InvalidStateError**，send 前查 `readyState !== WebSocket.OPEN`；③**resolve 后立即 close 会截断 session.finish 帧**：VAD 模式先发 session.finish 再关的要求在测试里表现为「服务端没收到 finish」，收口后 `setTimeout(close, 50)` 让帧 flush；④**服务端销毁 socket 时 undici 触发 `onerror` 而非 `onclose`**（连接没完全 open），错误码是 `provider-unreachable` 不是 `provider-closed`，测试断言要放宽；⑤**底噪采样率的坑**：TTS `sample_rate` 请求 16000 与采集链路同率可零重采样，但 qwen-tts 文档只说 24k 是「默认」不是唯一；⑥**host 路由改动后旧进程无感知**：改 `lib/index.js` 后运行中的 dsh web 不加载新路由（请求返回通用 401），验证部署必须重启 host 进程（工作区规则：dsh web 用户手动管理）。**验证**=`pnpm typecheck` 双 program、`pnpm build`、`node --test` 144/144（132+6+6）；共享浏览器：设置卡「实时服务商」下拉两档可选、「回复播报」云端 TTS 选项 + Cherry 音色字段出现；TTS host 路由 401 待用户重启后复验。可复现?是（build+test 可复现；真机转写/合成需 DASHSCOPE_API_KEY）。
- **dsh-asr-voice I4：client 云端实时引擎（PCM 上行 host 实时通道 + SSE 下行驱动字幕/回合）（2026-09-01，功能/实时通道）**：需求=路线表 I4：把 I3 的 host 实时通道接到浏览器——`capture.ts` 16k 采集帧上行，SSE 下行事件驱动字幕与回合，用假 provider 端到端验证闭环（真云端 provider 是 I5）。实现=①**`src/client/realtime-cloud.ts`**：第三档引擎 `createCloudRealtime`——采集帧 `peakAbs < 0.005` 静音守卫不上行（与整段模式同一判据），`floatToInt16Le` 量化后逐帧经**串行泵**上行（`upload` 是异步、回调里来帧是同步，必须排队逐帧发不能并发堆叠）；SSE 事件 `partial`→onPartial、`final`→onTurn（服务端 VAD 判回合，本地不再文字静默判定）、`error` 连续 3 次判死；会话生命周期含建连失败判死、迟到 sid 孤儿回收（`stop()` 后 createSession 才 resolve 的 sid 必须 `closeSession` 关掉）；pause 只停上行不关会话（`capture.setMuted(true)` + 清泵）、resume 恢复。②**`src/client/realtime-cloud-transport.ts`**：浏览器 fetch 实现 host 4 条 exact 路由；SSE 用 `fetch + ReadableStream` 逐行解析 `data: …`（不用 EventSource——后者无法精确 AbortController 关闭，且错误恢复控制更弱）。③接线：`realtime.ts` 分派加 `cloud`；config/settings/settings-card/locales 三档引擎同步；`voice-chat-button` 前置检查按引擎分派（cloud 只需 `isPcmCaptureSupported`，不需要 `cloudConfigured`——假 provider 不需要 key）。④测试 7 例（`test/realtime-cloud.test.mjs`）：建会话/上行 int16 字节数/静音守卫/多句各自成回合/pause 停上行不关会话/resume 恢复/stop 幂等关+判死/建连失败判死/孤儿 sid 回收。**坑**=①**采集回调同步、网络上行异步**：onFrame 里直接 `await fetch` 会堆叠请求，必须做串行泵（queue + inFlight 标志），否则帧积压；②**`language` 参数在 cloud 引擎 I4 阶段用不上**（host 实时通道暂不收语言），`noUnusedParameters` 下未使用参数直接删签名别留；③**`Uint8Array<ArrayBufferLike>` 当 `BodyInit` 会 TS2322**：与 transcribe 同坑，`fetch` body 前要 `new Uint8Array(pcm.byteLength)` 拷贝成 fresh `ArrayBuffer`；④**判死与 stop 的 closeSession 幂等**：failNow 已关会话，stop 再调 `closeSession` 要能幂等（测试断言 closeCount=1）；⑤**建会话是异步的，start 后立刻 stop 的竞态**：createSession resolve 时必须检查 `gen !== generation` 再决定是否关掉孤儿 sid。**验证**=`pnpm build` + `pnpm typecheck` 全过；`node --test` 132/132 全绿（新增 7 例）；README/REQUIREMENTS 标记 I4 已交付，I4 关键产出（Chromium AEC 消除率实测数字）留给真机采集。可复现?是（build+test 均可复现）。

- **dsh-asr-voice I3：host 实时通道（会话注册表 + SSE 带背压 + RealtimeProvider 接缝 + 假 provider）（2026-09-01，功能/实时管道）**：需求=路线表 I3（纯管道，用户不可感知）：host 实时转写通道——浏览器 PCM 上行 → RealtimeProvider 接缝 → SSE 下行，为 I4（接 capture.ts 验字幕播放）与 I5（真云端 provider）铺路；含「undici WebSocket 能带 Authorization」的真实 socket 上线证据测试。实现=①**`src/realtime-provider.ts`**：接缝类型（`RealtimeProvider.connect() → RealtimeProviderConnection`，`send(pcm)` 上行 / `onEvent` 事件下行 / `close()`）+ 假 provider（能量 VAD 把 int16 PCM 切成句，句内 partial → 句尾 final + speech-stopped，行为形状对齐 qwen3-asr-flash-realtime 真流式：先 partial 后 final、服务端 VAD 断句）；②**`src/realtime-host.ts`**：`RealtimeHost` 会话注册表（`sid` = `crypto.randomUUID()` host 铸造、不透明 token）+ `SseChannel`（partial coalesce / final 必达的背压 + 空闲心跳 + 断连清理）+ 4 条 exact 路由全过 `isTrusted`（`POST session` 建 / `POST audio?sid=` PCM 上行 / `GET events?sid=` SSE / `POST close?sid=` 关）+ 空闲超时自动拆会话 + SSE 挂起前的有界 pending 缓冲；③`index.ts` 用 `createFakeRealtimeProvider` 装配（I5 只换 `createProvider`）；④测试 19 例（provider 7 + host 10 + ws-auth 2）。**坑**=①**webserver `register` 对重复 (kind,path) 直接抛错**：同一路径不能挂 POST+DELETE 两个 method，4 条路由必须路径互异——关闭会话走独立的 `/realtime/close` 路径而非 `/session` 的 DELETE；②**`sendJson` 走 `res.end(body)` 不是 `res.write`**：测试替身 `end()` 也必须收 body，否则 `JSON.parse(res.body)` 拿到空串；③**FakeRes 背压不能是「第 N 次 write 后永久 false」**：真实内核缓冲是 drain 后恢复，测试要用「backed 布尔」+ 手动 drain，否则 final 在 drain 后二次背压被丢、断言全红；④**close() 里先置 closed 再 emit 会吞掉 final**：emit() 拒绝已关闭连接的事件，收尾事件必须先 emit 再置 closed；⑤**`speechWindows` 只数有声窗**（对齐 client VAD）：假 provider 把静音窗也计进 minSpeech 会让短杂音段误判成句；⑥**测试建了会话必须 close**：10 分钟空闲定时器会让 node --test 进程永不退出（背压测试失败未走到 closeSession 就挂进程）；⑦**空闲超时测试的 clock 必须会前进**：固定 clock + refreshIdle 无限重排定时器；⑧**ws-auth 测试 `server.close()` 会等已升级 socket**：不销毁 socket 就永远等不到回调，升级过的 socket 必须主动 destroy；⑨**undici WebSocket（Node 全局）能带 `Authorization` 头完成真实握手**（`new WebSocket(url,{headers})`）——浏览器 WebSocket 不能自定义头但 host 可以，这是 I5 qwen3 上行带 DSH 凭据的前置证据，已用本地真 WS 服务测通。**验证**=`pnpm build` + `pnpm typecheck` 全过；`node --test` 124/124 全绿（新增 19 例）；真实 http server + fetch 冒烟：建会话 → SSE 200 `text/event-stream` → 音+静音上行 → 事件流 `speech-started → partial×3 → speech-stopped → final` → close 200。可复现?是（build+test+冒烟均可复现）。
- **dsh-asr-voice：API key 迁入 DSH credentials + 设置卡重做为三步向导（2026-08-31，功能/安全/配置面）**：需求=修好「选不动」（见 fixes.md 同日条）之余，把卡片做到「看懂 → 会配 → 会换供应商 → 会放 key」。三项已拍板：key 存 DSH credentials、三步向导 + 高级折叠、由用户重启 `dsh web` 自点验收。
实现=①**key 通道**：新增零依赖 `src/key-ref.ts` 的 `keyRefFor({preset,name,id})`——预置直接沿用官方 LLM 的命名 `<PRESET>_API_KEY`（派生法与 `packages/client/ui-settings-models/src/client/store.ts` 的 `deriveKeyRef` 一致），自定义派生 `ASR_VOICE_<NAME>_API_KEY`；引用名必须过 `^[A-Za-z_][A-Za-z0-9_]*$`（`packages/host/apiproxy/src/api/credentials.schema.ts`），故非字母数字一律折叠成 `_`、纯中文名退到按行 id 派生（保证行间不撞车）。解析顺序：过渡期 settings 残留明文 → `ctx.get('credentials').resolve(ref)` → `process.env[ref]`，**顺手删掉了原先只给 MiMo 开洞的 `/xiaomimimo/i` baseUrl 特判**。②**settings 侧脱敏**：两个 `apiKey` 补 `.role('secret')`（字段本身本版本刻意留着，否则 schemastery 会把旧文档里的值剥走、迁移读不到），`CloudProviderSchema` 加 `name`；host `apply()` 内挂一次性迁移 effect，把任何非空 `apiKey` 逐把 `credentials.set(ref)`，全部成功后才回写清空 settings 明文——**任一把被拒即整批放弃**，绝不抹掉没落库的 key。③**client 半区连类型都不再有 apiKey**：`AsrVoiceConfig` 里不存在该字段，密钥只经 `connection.api.credentials.{describe,set,unset}` 的写视图（只问 configured / writable / source）。④**UI**：① 识别方式三 chip → ② 服务商 chip（点即联动填 baseUrl/model/mode，数据源仍是 `src/presets.ts`）→ ③ 密钥三态（复用成功 ✓ / 未配置 write-only 输入框 / 只读来源禁用）+ 统一「测试连接」，其余全部收进默认折叠的「高级」，能力不减；顶层常驻「未保存的更改 · 保存 / 放弃」条。
决策理由=①**「测试连接」= 列一次该端点的模型**（`/api/asr-voice/asr-models?providerId=`）而不是录一段音：不需要麦克风权限、不打扰用户，且一次验掉 key + baseUrl + 网络三件事，返回的模型还能直接灌进高级里的模型下拉；②**只有「保存」写宿主**：settings 段是整段替换，逐控件即时写会把一次误点扩大成整段脏写；③**key 必须先离开 settings，卡片才敢 draft 化**——`SettingsPathOp` 明确警告「拿脱敏后的文档重建整段写入会静默删掉 secret」，迁移后 `asr.cloud.providers[*]` 不含 secret，`scope.set('asr', …)` 的整段替换才是安全的（B 让 C 变安全的那一环，两块不能反序做）。
坑=①**`SettingsScope.set` 吞掉失败并重载宿主状态，promise resolve 不代表落盘**：唯一可信判据是写完读回 `getSnapshot().value` 比对（照 `ui-settings-plugins/src/client/card-form.ts`），且失败只能报到**段**这一级——这条路上拿不到原始拒绝文案（能拿到原文的是「测试连接」与 `credentials.set` 两条路）；②**读回比对的基准必须是宿主真相而不是本地快照**：`writeDraft` 自己会把草稿并进本地快照，以本地快照为基准会让「失败后再点一次保存」算出零变更从而**假报成功**（回归测试连点两次、要求两次都报同一段失败）；③凭据 value 是 `z.string().min(1)`，「清除」只能走 `unset`；网关对 caller **没有引用名白名单**，所以 `ASR_VOICE_` 前缀 + README 权限披露是唯一自律面，复用 `<PRESET>_API_KEY` 等于共用 LLM 那把 key 与配额（已在 README 明写）；④改自定义服务商的显示名会换引用名、旧 key 成孤儿，文案里显式警告；⑤client 没有 `dsh-credentials` / `dsh-client-connection` 的 value 依赖，凭据面按结构化 `*Like` 接口接（与既有 `CredentialsLike` 同法），别为此加包依赖；⑥原计划承诺「迁移被拒时 UI 显示原始错误」**未兑现**：DSH client 的 `SettingsScopeSnapshot` 根本不暴露脱敏 sidecar（`secrets[]` 只在 wire 与 host 侧），不改 DSH 源码就拿不到，故退成「host 一条 warn + 卡片显示该 ref 未配置 + 识别仍走残留明文」，README 按实际行为写清；⑦修正 8/27 那条「`mergeHostValue` 对 providers 要整表覆盖」的后半句——整表覆盖仍必要（深合并会残留已删供应商），但**覆盖前必须克隆/逐字段重建**，否则就是把宿主冻结引用请进可变快照。
验证=`pnpm run typecheck` 双 tsconfig 过；`pnpm test` 36/36（`test/config-freeze.test.mjs` 覆盖 keyRefFor 形状与非法字符、`readKeyState` 复用态、`saveKey` 去空白 + 只读来源读回拒绝时报错点名 ref、apiKey 永不出现在 `config`）；`bash scripts/build.sh` 产 `lib/`（新 `lib/key-ref.js`）无 purity gate 违规；grep 自查 `src/client/**` 无对 providers 行的原地赋值；README 的预置表补「凭据引用名」列、权限表补「凭据读写」行、新增「快速开始」节。用户手点验收清单（8 步）已交付，等待其实测。可复现?不适用（新功能；等价性检查见验证）。

- **dsh-asr-voice 五项新需求：文本模式/剪贴板/性能/用量统计/云端多供应商（2026-08-27，功能/增强/协议）**：问题=用户提 5 项需求。解法=①**文本输入模式** `behavior.textMode`（`replace` 完整替换草稿默认 / `append` 末尾追加）——append 用 slot 传入的 `props.input?.draft` 读当前草稿（InputActions 无 getter，draft 在 owner props 的 InputState 里），按空格/换行分隔拼接；②**自动复制剪贴板** `behavior.copyToClipboard`（默认 true）——finalize 后 `navigator.clipboard?.writeText(text).catch(()=>{})`（localhost 是安全上下文，API 可用；静默容错）；③**非 ASR 速度**——启发式快速路径（`heuristicOptimize(text)===text` 即本就干净 → 跳过 LLM 优化直接填入，省一次模型调用）+ optimize/transcribe 客户端 AbortController 超时 + WAV 转码复用懒加载 AudioContext；④**ASR 用量统计**（低优先级）——host 进程内计数器（count/chars/lastAt/lastProvider）+ `/api/asr-voice/stats` + 设置页展示（重启清零，仅参考）；⑤**云端多供应商**——schema 重构 `asr.cloud.providers[]`（每项 id/preset/baseUrl/apiKey/model/mode）+ `active`，兼容旧单配置（providers 空且 legacy baseUrl 有值时 host 回退 legacy；设置页挂载时自动迁移为 providers[0] 并写回）；每供应商「获取模型」按钮 → 新路由 `/api/asr-voice/asr-models?providerId=X`（用该供应商 key 调 `GET {baseUrl}/models`，按 /asr|audio|omni|whisper|sensevoice|voice|transcri/ 过滤 ASR 模型）。**坑**=①**schemastery 的 `z.object` 里 `z.array(对象schema).default([])` 会让声明发射报 TS2883**（推断类型引用 cosmokit 的 `Dict`，不可移植）——`Schema` 类型未从模块导出（`export default` 只导出 const），`Schemastery.Schema` 全局命名空间也没有；解法=导出 schema 注解 `any` + 手写 `AsrVoiceSettings` 业务接口（schema 只做运行时校验，业务类型手写兜底）；②`mergeHostValue` 对 `providers` 数组要**整表覆盖**（深合并会残留已删供应商）；③MiMo 的 `apiKeyEnv` key 不在 process.env、要经 `ctx.get('credentials')` 解析（前条已记），多供应商下每项各自解析；④api.openai.com 从国内网络**直连超时**（10s UND_ERR_CONNECT_TIMEOUT）——OpenAI/Groq 在这类网络不可用，国产端点（xiaomimimo/dashscope/siliconflow）才行，多供应商功能正是为此（存多家、切能连通的）。**验证**=host/client tsc 全过；build 产 host 新模块（stats.js/asr-models.js）+ client 99.7KB；host 模块 import 加载 OK、createAsrStats 记录/快照正确；asr-models 导出 registerAsrModelsRoute。可复现?是（TS2883 用数组对象 schema default 必现；api.openai.com 国内直连必超时）。

- **dsh-asr-voice 预览卡可读性修复：SVG 默认 300px 巨图标 + 删除线看不清（2026-08-27，UI/修复）**：问题=识别优化预览弹窗「看不清文字 + 一个大大的麦克风」。原因=①预览卡标题里的 `<MicIcon />` 是 `viewBox="0 0 24 24"` 且**无 width/height**——浏览器对无尺寸内联 SVG 默认渲染约 300px（只有 `.dshav-mic-button svg` 显式约束了按钮内图标，预览卡标题里的没约束）；②原始转写样式 `text-decoration: line-through`——用户说的原话被删除线划掉，视觉上像已删文本，难读。解法=①`.dshav-preview-title svg { width:16px; height:16px; display:block }` 显式约束；②去掉 line-through，原始文本用次级色（--dshav-text-2）放在浅底块里，优化后文本主色+medium 加粗、区块用 accent 6% 底轻强调；预览改 block 布局（两个带底色的信息块）+ 入场动画。**坑**=①**内联 SVG 不给 width/height 时浏览器按默认可替换元素尺寸渲染（约 300px）**——任何在尺寸容器之外使用的图标都必须显式给 CSS 尺寸，别只依赖容器撑；②「原始 vs 优化」对比里删除线是错误隐喻（原始不是被删除，是待替换），用层级/底色区分比划线清晰。**验证**=client tsc+build；fetch 服务器 `/plugins/dsh-asr-voice/client.js` 含 dshav-preview-block 且无 line-through、标题 svg 16px。可复现?是（无尺寸 SVG 在预览标题必 300px）。

- **dsh-asr-voice 云端 ASR 双通道：chat+input_audio + 小米 MiMo 预置（2026-08-27，功能/协议探测）**：问题=用户选定小米 MiMo（`mimo-v2.5-asr`）做云端 ASR，但插件只有 whisper 式 multipart `/audio/transcriptions` 通道；且旧配置 DashScope `qwen3-asr-flash` 同样是「音频大模型」不走 whisper。原因=web 核实官方文档（mimo.mi.com/docs 语音识别 + 阿里云百炼 qwen-asr-api-reference）：**MiMo-V2.5-ASR 与通义 qwen3-asr-flash 的 OpenAI 兼容姿势都是 `chat.completions` + `messages[].content[].input_audio`（base64 data URI），不是 `/audio/transcriptions`**。解法=①host /transcribe 双通道：`transcriptions`（原 multipart）+ `chat`（POST `{baseUrl}/chat/completions`，body `{model, messages:[{role:user,content:[{type:'input_audio',input_audio:{data:'data:{mime};base64,…'}}]}], asr_options?:{language}}`，解析 `choices[0].message.content` 并剥 `<chinese>` 等语言标签）；②`mode=auto`（默认）按模型名判定：模型名含 `asr/audio/omni` 或 `sensevoice` → chat，否则 transcriptions；③预设表加 **小米 MiMo**（baseUrl `api.xiaomimimo.com/v1`，`mimo-v2.5-asr`，mode chat），DashScope 预设 mode 改 chat；④settings 加 `asr.cloud.mode` + 设置卡「调用通道」下拉；⑤**MiMo key 复用**：baseUrl 含 xiaomimimo 且设置 apiKey 空时，host 走 `ctx.get('credentials').resolve('MIMO_API_KEY')`（DSH 官方 credentials 服务，与 mimo LLM provider 同一把 key，存 `~/.dsh/.credentials.yaml`），再兜底 process.env。**坑**=①上游语言参数要映射（DSH 的 `zh-CN/en-US/auto` → 上游 `zh/en/auto`，别原样传）；②chat 响应 content 可能带语言标签，`content.replace(/<[^>]+>/g,'')`；③`credentials.resolve` 的 ref 是 branded string，运行时传裸字符串即可（类型是编译期 brand）；④`apiKeyEnv` 类 key **不在 process.env**，DSH 是在请求时经 credentials 服务解析（dsh-llm-pi-ai index.js:2412），插件要复用就得走 `ctx.get('credentials')` 而非读 env。**验证**=typecheck+build 全过；用 .credentials.yaml 的 MIMO key 发 1s 静音 WAV 冒烟 → HTTP 200、content="嗯。"（鉴权/通道/模型全通，静音音频可识别出语气词）。可复现?是（任意有效 MIMO key + 静音 WAV POST chat/completions 必 200）。
- **dsh-asr-voice 引擎 auto：浏览器优先 + 云端自动兜底 + 错误暴露（2026-08-27，功能/增强）**：问题=默认引擎 `browser`（Chrome Web Speech）在国内被 Google 识别服务器屏蔽，识别静默失败；且旧代码对 `network/no-speech` 错误**不 settle 不提示**，UI 卡在「录音中」。解法=①引擎新增 `auto`（默认，schema default 改 auto）：Web Speech 可用时优先，失败（`network/not-allowed/service-not-allowed/不支持`）且云端已配置 → 自动切云端重试一次并提示「已自动切换云端 ASR」；②Web Speech `onerror` 全部先 `settle()`（防挂起）+ 错误码映射（`network`→明确文案「被网络屏蔽请改用云端」、`no-speech`→轻提示「未检测到语音」、`mic-denied`→权限提示）；③云端录音 `pickMime` 优先 `audio/mp4`（m4a，DashScope/MiMo 兼容性优于 webm）+ 转写 fetch 加 60s AbortController 超时；④`cloud-not-configured` 前置预检（开始录音前就报，别录完才 502）。**坑**=①auto 兜底要排除 `mic-denied`（用户拒绝麦克风时别自动换云端再弹权限，权限体系不同会连环弹）；②兜底只试一次（fallback 闭包内换引擎后同一错误回调不再重入）。**验证**=typecheck+build；bundle 含 fallbackToCloud/errWebSpeechNetwork/noSpeechDetected 文案。可复现?是（国内网络 Web Speech 必 network 错误）。
