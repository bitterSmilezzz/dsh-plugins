# 安装 / 部署（27 条）
- **dsh-shortcuts 发版 0.1.7：rc.1 兼容性复查结论 + align 未推提交顺带补推（2026-09-03，发版/兼容性复查）**：问题=用户说「今天更新了 0.1.2-c1，看看插件要不要做兼容性优化」。排查=①**版本核对**：npm registry raw API 与 GitHub Releases 均无 `0.1.2-c1`，官方今日（09-03 06:06 UTC）发布的是 `dsh-v0.1.2-rc.1`（GitHub Release + npm dist-tag `next` 双确认）——「c1」是 rc.1 的口语化；②六个自研仓（asr-voice/model-selector/notify/retry-settings/shortcuts/email）node_modules 实装版本全部已是 0.1.2-rc.1（9 月初 align 提交已落地），无需再升依赖；③typecheck（双 program）+ build 全绿证明 rc.1 的 API 与插件代码无破坏性冲突，唯一问题=shortcuts `focus-modes.ts` 遗留死代码 `sidebarEl`/`composerEl`（TS6133，改版后用 body 类方案后从未引用）。解法=删两函数 + 重建同步 lib 产物 → 常规发版流（bump 0.1.6→0.1.7 仅动 version 行 → typecheck/build/test 7/7 全绿 → split 提交 fix+chore → tag → push → clone tag 发布态验证 validate 20/20 + imports 0 缺失 → `gh release create`）。坑=①**align 提交 5128a6e 上次未 push（`git log origin/main..HEAD` 一查即现）**——发版 push 顺带补推，main 从 6c11305 直接跳到 54e90af；发布前务必核对本地未推提交，避免 tag 携带本地独有历史；②npm view 公共 registry 对 `dsh-shortcuts` 404 属常态——六仓全部从未发布到公共 npm（publishConfig 形同虚设），本体系「发版=version bump + git tag + GitHub Release」，publish 动作不需要 npm token；③经验文档「遗留（排期）」项（A1 combobox/listbox 重构、P2 虚拟化、E4 裁剪）均属 dsh-vision-bridge / dsh-web-search-free 等非本工作区仓，复查勿混淆。结论=rc.1 对自研六仓零破坏；兼容性复查=版本核对 + 实装版本 + typecheck 三件套，不必等运行时。可复现?是（`git log origin/main..HEAD` 抓未推提交；clone tag + validate-plugin.mjs 复验发布态）。
- **六仓 tag↔Release 对齐大审计：23 个缺失 Release 一次性回填 + 审计脚本的两个假阳性来源（2026-09-02，发版/Release 治理）**：问题=用户要求「release 和 tag」对齐。正确盘点（`gh release list --json tagName --jq '.[].tagName'` 对比 `git tag -l`）发现 notify×6/retry-settings×5/shortcuts×4/email×8 共 23 个 tag 只有 git tag 没有 Release（asr-voice/model-selector 已对齐）。解法=逐 tag 生成 notes（`git log <prev>..<tag>` 区间提交列表 + 「历史回填」说明）批量 `gh release create`；**回填一律 `--latest=false`，仅仓库最新 tag 走默认 Latest**（09-01 教训：Latest=发布时间最近的非 pre-release，倒序回填会让旧版抢走 Latest）。**坑**=①**审计脚本第一版假阳性**：`gh release list` 的第一列是**标题**不是 tag——带空格自定义标题（如「dsh-asr-voice · v0.1.0（开口即成文 Voice Input）」）让 `awk '{print $1}'` 抓到标题首词，误报一片「缺失」；必须用 `--json tagName --jq`；②**gh 在多 remote 仓库默认解析到第一个 remote**——dsh-email 有 origin（用户 fork bitterSmilezzz）+ upstream（STARDUSTLC666），release 操作被解析到上游 → 403 Resource not accessible；必须显式 `-R bitterSmilezzz/<repo>`；③**zsh 不对未加引号的变量做词分割**：`R="-R repo"; gh ... $R ...` 在 zsh 整体成一个词报 GraphQL 找不到仓库（bash 才分词）——验证脚本要么用数组要么显式写参数；④422 tag_name already exists 反而是保护（防止对已有 Release 重复创建）。验证=六仓 comm 差集全空、Latest 各自正确（model-selector v0.1.13/asr-voice v0.2.6/notify v0.1.5/retry-settings v0.1.5/shortcuts v0.1.3/email v0.8.5）。可复现?是（各仓 `git tag -l | sort` 与 `gh release list --json tagName` 差集即审计）。
